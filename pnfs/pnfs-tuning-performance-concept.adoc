...
[NOTE]
====
默认情况下，pNFS 要求 SVM 中的任何数据 LIF 都可路由到 NFS 客户端上的接口，因为 pNFS 设备列表将填充 SVM 中的任何数据 LIF。因此，可能会选择不可路由的数据 LIF，这可能会造成中断情况。最佳实践是，仅在使用 pNFS 时配置可路由数据 LIF。

从ONTAP 9.18.1 RC1 及更高版本开始，您可以按子网指定哪些接口符合 pNFS 流量的条件，从而允许混合使用可路由接口和不可路由接口。请联系NetApp支持部门获取有关命令的信息。
