...
+
以下命令将创建一个名为 `policy1`的策略，该策略具有名为 `event1`的事件以及名为 `engine1`的外部引擎。此策略使用策略配置中的默认值：
`vserver fpolicy policy create -vserver vs1.example.com -policy-name policy1 -events event1 -engine engine1`

+
以下命令将创建一个名为  `policy2` 的策略，该策略具有名为  `event2` 的事件以及名为  `engine2` 的外部引擎。此策略配置为使用指定的用户名进行特权访问。已启用 Passthrough-read：
...
+
以下命令将创建一个名为 `native1`的策略，该策略具有名为 `event3`的事件。此策略使用本机引擎，并在策略配置中使用默认值：
