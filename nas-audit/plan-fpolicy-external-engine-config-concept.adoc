...
指定发送到外部引擎的Fpolicy通知消息的格式。此参数的默认值为`xml`。

从ONTAP 9.11.1开始、在指定`protobuf`时、可以使用Google Protobuf以二进制格式对通知消息进行编码。在设置`protobuf`之前、请确保Fpolicy服务器也支持Protobuf反序列化。
 a| 
