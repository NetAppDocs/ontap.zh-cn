...
summary: 当暂存卷或包含审计事件日志的卷上空间不足时，可能会出现问题。如果空间不足，则无法创建新的审计记录。启用审核保证后，可能会阻止客户端访问数据并导致访问请求失败。您应该了解如何排查和解决这些卷空间问题。 
---
...
[role="lead"]
当暂存卷或包含审计事件日志的卷上空间不足时，可能会出现问题。如果空间不足，则无法创建新的审计记录。启用审核保证后，可能会阻止客户端访问数据并导致访问请求失败。您应该了解如何排查和解决这些卷空间问题。

[NOTE]
====
此行为在启用审核保证时适用。有关详细信息，请参见link:../nas-audit/auditing-process-concept.html["有保障的审核"]。
...
如果包含事件日志文件的卷空间不足，审核无法将日志记录转换为日志文件。启用审核保证时，会导致客户端访问失败。您必须知道如何解决与事件日志卷相关的空间问题。
...
如果包含存储虚拟机 (SVM) 暂存文件的任何卷空间不足，审核将无法将日志记录写入暂存文件。启用审核保证时，这将导致客户端访问失败。要解决此问题，需要通过显示有关卷使用情况的信息来确定 SVM 中使用的任何暂存卷是否已满。
