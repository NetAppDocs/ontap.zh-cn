...
从 ONTAP 9.19.1 开始，当您有数百到数千个客户端同时列出相同的小型 NAS 目录（2MB 或更少）（约 25,000 个文件）时，性能会提高。这种增强功能减少了使用本地 FlexCache 卷等变通方法来支持高客户端数执行目录列表的需要。对于较低的客户端数量，性能不会受到负面影响。ONTAP 9.19.1 允许更多 CPU 资源处理小型 NAS 目录的列表请求。此性能增强是自动的，不需要配置。
