...
summary: 从 ONTAP 9.7 开始，您可以使用原生 ONTAP 审核以及通过 FPolicy 管理文件策略来审核 FlexCache 关系中的 NFS 文件访问事件。使用 SMB 的 FlexCache 卷不支持 FPolicy 。原生审核和 FPolicy 使用与 FlexVol 卷相同的命令行界面命令进行配置和管理。但是， FlexCache 卷存在一些不同的行为。 
---
= 审核 FlexCache 卷的注意事项
:allow-uri-read: 
:icons: font
...
[role="lead"]
从 ONTAP 9.7 开始，您可以使用原生 ONTAP 审核以及通过 FPolicy 管理文件策略来审核 FlexCache 关系中的 NFS 文件访问事件。使用 SMB 的 FlexCache 卷不支持 FPolicy 。原生审核和 FPolicy 使用与 FlexVol 卷相同的命令行界面命令进行配置和管理。但是， FlexCache 卷存在一些不同的行为。
