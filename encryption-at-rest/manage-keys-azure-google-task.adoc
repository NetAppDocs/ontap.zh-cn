...
* 您必须已在相应的云提供程序上配置KMS。
* ONTAP集群的节点必须支持NVE。
* link:https://docs.netapp.com/us-en/ontap/encryption-at-rest/install-license-task.html["您必须已安装卷加密(VE)和多租户加密密钥管理(MTEKM)许可证"]。这些许可证包含在中 link:https://docs.netapp.com/us-en/ontap/system-admin/manage-licenses-concept.html#licenses-included-with-ontap-one["ONTAP One"]。
* 您必须是集群或SVM管理员。
