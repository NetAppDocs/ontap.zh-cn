...
. 修改主密钥服务器，添加辅助密钥服务器。这 `-secondary-key-servers` 该参数接受以逗号分隔的最多三个密钥服务器列表。辅助密钥服务器无需提供端口号。
...
--
. 修改主密钥服务器，添加辅助密钥服务器。这 `-secondary-key-servers` 该参数接受以逗号分隔的最多三个密钥服务器列表。辅助密钥服务器无需提供端口号。
`security key-manager external modify-server -vserver _svm_name_ -key-servers _primary_key_server_ -secondary-key-servers _list_of_key_servers_`有关辅助密钥服务器的更多信息，请参阅 <<mod-secondary>>。
...
辅助密钥服务器由以下方式管理： `-secondary-key-servers` 参数 `security key-manager external modify-server` 命令。这 `-secondary-key-servers` 参数接受以逗号分隔的列表。列表中辅助密钥服务器的指定顺序决定了辅助密钥服务器的访问顺序。可以通过运行以下命令来修改访问顺序 `security key-manager external modify-server` 辅助密钥服务器的输入顺序不同。辅助密钥服务器无需提供端口号。
