...
Onboard Key Manager 在集群内部的安全数据库中存储和管理密钥。它的作用域是集群。外部密钥管理器在集群外部存储和管理密钥。其范围可以是集群或存储 VM。

.关于此任务
您必须使用 ONTAP 命令行界面 (CLI) 为 MetroCluster 配置配置外部密钥管理器。了解 link:configure-external-key-management-concept.html["使用 CLI 配置 ONTAP 外部密钥管理"]。

可以使用一个或多个外部密钥管理器。以下条件适用：
