...
+
xref:assign-authentication-keys-seds-external-task.adoc[将数据身份验证密钥分配给 FIPS 驱动器或 SED （外部密钥管理）]

. 如有必要，请为 FIPS 驱动器分配 FIPS 140-2 身份验证密钥。
+
xref:assign-fips-140-2-authentication-key-task.adoc[将 FIPS 140-2 身份验证密钥分配给 FIPS 驱动器]
