...
从 ONTAP 9.15.1 开始，SnapMirror 主动同步支持在故障转移事件后在扇出支路中自动重新配置。如果从主站点到辅助站点的故障转移成功，则第三站点将自动重新配置，以将辅助站点视为源站点。异步扇出分支可以是 SAN 配置中的一致性组关系，也可以是独立的卷关系。重新配置将适用于任一情况。重新配置由计划内或计划外故障转移触发。故障回复到主站点时也会发生重新配置。
