...
从 ONTAP 9.19.1 开始，AIX 的透明应用程序故障转移在对称主动/主动配置中通过 SnapMirror 主动同步得到支持。您可以使用 System Manager 或 ONTAP CLI 来创建 AIX LUN。为确保透明故障转移，使用 ONTAP CLI 时，必须使用选项 access-mode local 和 lun create 或 lun modify 命令。默认情况下，访问模式设置为 global。使用 System Manager 时，选择 AIX 作为主机，local 作为访问模式。有关创建 LUN 的详细信息，请参见 link:../san-admin/provision-storage.html["配置 SAN 存储"]。
