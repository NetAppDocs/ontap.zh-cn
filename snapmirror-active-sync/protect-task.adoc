...
---
= 使用适用于 SAN 的 ONTAP SnapMirror 活动同步进行保护
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
使用 SnapMirror 活动同步为 SAN 配置配置非对称保护涉及在 ONTAP 源集群上选择 LUN 并将其添加到一致性组。
...
您可以使用 System Manager 或 ONTAP CLI 为 SAN 配置建立对称保护。在两个界面中，都有不同的步骤 xref:index.html#key-concepts[统一和非统一配置]。
...
--
.统一配置的步骤
. 在主站点上，link:../consistency-groups/configure-task.html#create-a-consistency-group-with-new-luns-or-volumes["使用新 LUN 创建一致性组"]。
+
