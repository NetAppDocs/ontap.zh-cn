...
* 删除 SnapMirror 活动同步 SAN 关系之前，必须先取消映射目标集群中的所有 LUN。
...
. 在*删除复制配置*窗口中，选中*释放源卷基本快照。*并选择*删除*。
...
. 在源集群上，导航到*保护>复制*。
. 找到要删除的关系，选择image:icon_kabob.gif["菜单选项图标"]并选择*删除*。
. 在 *Delete replication configuration* 窗口中，选中 *Release the source storage VM base snapshots.* 并选择 *Delete*。
