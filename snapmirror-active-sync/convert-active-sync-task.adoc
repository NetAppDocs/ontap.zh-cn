...
---
= 将现有 ONTAP SnapMirror SAN 关系转换为适用于 SAN 关系的 SnapMirror 活动同步
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
如果您已配置 SnapMirror 保护，则可以将 SAN 关系转换为适用于 SAN 的 SnapMirror 活动同步。从 ONTAP 9.15.1 开始，您可以将关系转换为使用对称主动/主动保护。

有关将 SVM-DR 关系转换为适用于 NAS 关系的 SnapMirror 活动同步的信息，请参见 link:protect-nas-task.html#convert-an-svm-dr-relationship-to-a-snapmirror-active-sync-relationship["将 SVM DR 关系转换为 SnapMirror 活动同步关系"]。
