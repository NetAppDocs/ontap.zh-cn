...
A系列、C系列
 a| 
自动故障转移双工（对称 Active/Active）、自动故障转移（非对称 Active/Passive）
 a| 
* 2（ONTAP 9.9.1 或更高版本）
...
* 在早于 ONTAP 9.15.1 的版本中，读写目标卷（在非对称主动-被动模式下从 DP 转换为读写的卷）不支持 SnapMirror 活动同步关系。在使用读写卷之前，必须通过创建卷级 SnapMirror 关系（异步或同步），然后删除该关系，将其转换为 DP 卷。有关详细信息，请参见link:convert-active-sync-task.html["将现有SnapMirror关系转换为SnapMirror活动同步"]。
