...
summary: 如果要通过在 SAN 配置中添加或删除卷来更改一致性组的组合，则必须首先删除原始关系，然后使用新组合再次创建一致性组。 
---
= 向ONTAP一致性组添加或删除卷
:hardbreaks:
:allow-uri-read: 
...
[role="lead"]
随着应用程序工作负载要求的变化，您可能需要从一致性组中添加或删除卷，以确保 SAN 配置中的业务连续性。在活动 SnapMirror 活动同步关系中添加和删除卷的过程取决于您正在使用的 ONTAP 版本。
