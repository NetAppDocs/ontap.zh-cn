...
. 在两个集群中的每个节点上至少为您的 SAN 配置创建一个 SAN LIF（如果适用，则为 FC 或 iSCSI），或者在主集群中的每个节点上至少创建一个 NAS LIF（如果适用，则为 NFS 或 SMB），以便复制到辅助集群。
+
...
+
link:../nfs-config/index.html["了解 NFS 配置"]

+
link:../smb-config/index.html["了解 SMB 配置"]

. 仅限 SAN：创建必要的 LUN 并将其映射到 igroup，igroup 用于将 LUN 映射到应用程序主机上的启动器。
+
link:../san-admin/provision-storage.html["创建 LUN 并映射 igroup"]

. 重新扫描应用程序主机以发现任何新的 LUN 或卷。
