...
使用 ONTAP Mediator 监视 SnapMirror 活动同步关系时，如果没有有效的自签名或证书颁发机构 (CA) 证书，则无法在 ONTAP 中初始化 ONTAP Mediator。您可以将有效证书添加到对等集群的证书存储中。使用 ONTAP Mediator 监控 MetroCluster IP 系统时，初始配置后不使用 HTTPS；因此，不需要证书。
...
-----END CERTIFICATE-----
----
. 将 ONTAP Mediator CA 证书添加到 ONTAP 集群。出现提示时，插入从 ONTAP Mediator 获得的 CA 证书。在所有对等集群上重复这些步骤：
+
