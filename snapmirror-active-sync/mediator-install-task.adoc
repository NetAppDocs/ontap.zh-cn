...
* ONTAP Mediator 1.12.1 或更高版本安装在具有双核 CPU 的主机上。
* 已使用 HTTPS 协议配置 ONTAP Mediator 实例。
+
...
IMPORTANT: 如果您在 ONTAP 9.17.1 中使用 SnapMirror 主动同步和 ONTAP Mediator 或 ONTAP Cloud Mediator，您应该查看 link:https://library.netapp.com/ecm/ecm_download_file/ECMLP2492508["《 ONTAP 发行说明》"^] 中的*已知问题和限制*，以获取有关这些配置的重要信息。
...
使用 ONTAP Mediator 监视 SnapMirror 活动同步关系时，如果没有有效的自签名或证书颁发机构 (CA) 证书，则无法在 ONTAP 中初始化 ONTAP Mediator。您可以将有效证书添加到对等集群的证书存储中。
