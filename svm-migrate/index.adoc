...
| MetroCluster SVM | ONTAP 9.16.1. | 您可以将SVM从非MetroCluster HA对迁移到MetroCluster配置、也可以将MetroCluster配置迁移到非MetroCluster HA对。您不能将SVM从一个MetroCluster配置迁移到另一个MetroCluster配置。[注意]在ONTAP 9 16.1之前的版本中、SVM迁移不支持MetroCluster SVM迁移。您可以使用SnapMirror异步复制到link:https://www.netapp.com/media/83785-tr-4966.pdf["迁移MetroCluster配置中的SVM"^]。请注意、在MetroCluster配置中使用SnapMirror异步迁移SVM是一种_颠覆性_迁移方法。 


| NetApp 聚合加密（ NAE ） | 不支持 | 使用NAE的任何端点均不支持迁移。 
