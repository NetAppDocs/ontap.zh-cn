...
NOTE: 在混合集群中，每个 SVM 可迁移的最大卷数取决于支持卷数较少的集群成员。
...
* 源集群和目标集群都必须至少有一个接口可以访问所有迁移的 SVM 网络，否则迁移预检查将失败。
...
* link:../nas-audit/enable-disable-auditing-svms-task.html["必须在目标集群上启用审计日志。"]。
...
| 云实例 | 不支持  a| 
不支持将 SVM 从本地实例迁移到云端或从本地实例迁移到云端。
...
| ONTAP Select | 不支持 | 不支持将 SVM 迁移到 ONTAP Select 实例或从ONTAP Select实例迁移 SVM。 
...
| SMB协议 | ONTAP 9.12.1 但是不支持 SMB1 协议。  a| 
...
| 适用于 VMware vSphere 的 ONTAP 工具 | 不支持 |  


| 卷克隆 | 不支持 |  


| 垂直扫描 | 不支持 | 不支持迁移启用 Vscan 的 SVM。 
...
* 迁移后，本地快照策略会带有 -MIG 扩展名，这可能会破坏某些自动化功能。您应该搜索此扩展程序并重命名所需的策略。
* 迁移到 Vault 目标SnapMirror卷必须执行重新同步才能重新激活保护。需要重新同步存储库，因为迁移会在迁移 SVM 之间创建一个新的基线，该基线比存储库之前的基线更新。执行重新同步操作会重新激活存储库，同时也会删除比当前存储库基线更新的任何数据，特别是迁移期间生成的快照。
