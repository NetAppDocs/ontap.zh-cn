...
NOTE: 从AFF集群迁移到具有混合聚合的FAS集群时，自动卷放置将尝试执行类似的聚合匹配。例如，如果源集群有 60 个卷，则卷放置操作将尝试在目标集群上找到AFF聚合来放置这些卷。当AFF聚合空间不足时，卷将被放置在具有非闪存磁盘的聚合上。
...
* 目标上必须有足够的空间用于放置卷。
* 如果源 SVM 具有加密卷，则必须在目标上的集群级别配置板载密钥管理器或外部密钥管理。
+
** 在这种情况下，在源服务器上的 SVM 级别配置的密钥管理器将不会迁移到目标服务器上。目标服务器将使用集群级别的密钥管理器。
