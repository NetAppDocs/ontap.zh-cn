...
| SnapMirror异步复制到云关系 | ONTAP 9.12.1 | 从ONTAP 9.12.1开始、在迁移具有SnapMirror副本到云关系的SVM时、目标集群必须安装"复制到云"许可证、并且必须具有足够的可用容量来支持将要镜像到云的卷中的容量移动。 
...
| 防篡改Snapshot副本锁定 | ONTAP 9.14.1 | 防篡改Snapshot副本锁定不等同于SnapLock。SnapLock仍不受支持。 
