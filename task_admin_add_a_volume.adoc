...
+
** *通过 NFS 导出*：使用授予用户对数据的完全访问权限的 `default`导出策略创建卷。
...
NOTE: 您还可以将此卷的规格保存到 Ansible Playbook。有关更多详细信息，请转到 link:task_use_ansible_playbooks_add_edit_volumes_luns.html["使用 Ansible 攻略手册添加或编辑卷或 LUN"]。
