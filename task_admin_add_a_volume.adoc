...
summary: 您可以创建 NFS 卷并将其添加到为 NFS 或 SMB/CIFS 服务配置的现有 Storage VM 。 
---
= 添加卷
:toc: macro
:hardbreaks:
...
[role="lead"]
您可以创建 NFS 卷并将其添加到为 NFS 或 SMB/CIFS 服务配置的现有 Storage VM 。

image:workflow_admin_add_a_volume.gif["添加卷任务的工作流图"]

集群中应存在为 NFS 或 SMB/CIFS 服务配置的 Storage VM 。
