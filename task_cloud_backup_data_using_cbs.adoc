...
在将集群注册到 Cloud Manager 后，您需要启用 Cloud Backup Service 并启动首次到云的备份。
...
== 编辑用于 Cloud Backup Service 的保护策略

您可以更改 Cloud Backup Service 使用的保护策略。
...
. 选择 Cloud Backup Service 策略类型。
. 如果未启用 Cloud Backup Service ，请选择 * 启用 Cloud Backup Service * 。
