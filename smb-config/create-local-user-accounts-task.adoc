...
.步骤
. 创建本地用户： `vserver cifs users-and-groups local-user create -vserver vserver_name user-name user_name optional_parameters`
+
以下可选参数可能有用：
...
. 输入本地用户的密码，然后确认该密码。
. 验证是否已成功创建此用户： `vserver cifs users-and-groups local-user show -vserver vserver_name`
