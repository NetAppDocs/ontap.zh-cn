...
您需要在组织单位 (OU) 上具有以下最低权限才能创建 SMB 服务器：
...
* 已验证对 DNS 主机名的写入
* 已验证写入服务主体名称
* 读取 msDS-SupportedEncryptedTypes
* 写入 msDS-SupportedEncryptedTypes
...
* 您可以选择使用  `-ou` 选项将 SMB 服务器添加到其他 OU。
* 从 ONTAP 9.13.1 开始，您可以为 SMB 服务器配置默认站点。当 LIF 没有站点成员资格时，ONTAP 可以使用此站点；在查找受信任域域控制器时，如果受信任域的站点发现失败，ONTAP 也可以使用此站点作为备用。
...
. 在 AD 域中创建 SMB 服务器： `+vserver cifs create -vserver vserver_name -cifs-server smb_server_name -domain FQDN [-ou organizational_unit][-default-site site_name][-netbios-aliases NetBIOS_name, ...][-keytab-uri {(ftp|http)://hostname|IP_address}][-comment text]+`
+
加入域时，此命令可能需要几分钟才能完成。
...
从 ONTAP 9.13.1 开始，如果 ONTAP 无法发现受信任域的站点信息，SMB 服务器默认站点也将用作备用站点。
