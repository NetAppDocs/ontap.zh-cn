...
在执行受保护的操作之前，MAV 规则会被评估。这意味着规则会根据系统的当前状态执行。例如，如果为以下对象创建了 MAV 规则：  `volume modify`查询 `-size 5GB` ， 使用 `volume modify`将 5GB 卷大小调整为 2GB 需要 MAV 批准，但将 2GB 卷大小调整为 5GB 则不需要。
...
* link:https://docs.netapp.com/us-en/ontap-cli/search.html?q=storage+aggregate["存储聚合"^]
