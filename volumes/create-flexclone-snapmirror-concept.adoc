...
---
= 了解如何从 SnapMirror 源或目标卷创建 ONTAP FlexClone 卷
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
您可以从现有卷 SnapMirror 关系中的源卷或目标卷创建一个 FlexClone 卷。但是，如果您使用的是早于 ONTAP 9.12.1 的 ONTAP 版本，并且类型为 DP 的关系使用 `DPDefault` 策略，这样做可能会导致后续的 SnapMirror 复制操作无法成功完成。
...
如果将 XDP 关系与 ONTAP 9.12.1 或更高版本的 `MirrorAllSnapshots`策略一起使用，则不应遇到此问题。

相关信息：link:../data-protection/convert-snapmirror-version-flexible-task.html["将现有的ONTAP SnapMirror DP类型关系转换为XDP"]
