...
* 从ONTAP 9.18.1 开始，您可以将 QoS 策略分配给包含在具有 QoS 策略的其他对象中的对象，例如 SVM 中包含的卷。使用嵌套 QoS 策略时，将应用限制性最强的策略。
+
在ONTAP 9.18.1 之前的版本中，您可以将 QoS 策略分配给包含在其他具有 QoS 策略的对象中的对象，但这些对象由 QoS 单独跟踪，并且不会相互交互。例如，如果您有一个 50 IOPS 上限的 SVM 和一个 100 IOPS 上限的 FlexVol volume，则FlexVol volume受 100 IOPS 上限的限制，但 QoS 不会将其计入 SVM 工作负载。
