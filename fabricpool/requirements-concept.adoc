...
[NOTE]
====
如果备份应用程序使用 NDMP 以外的协议，例如 NFS 或 CIFS ，则性能层中备份的所有数据都会变得很热，并可能影响将该数据分层到云层。非 NDMP 读取可以将发生原因数据从云层迁移回性能层。
...
+
您不能使用 Flash Pool 聚合（同时包含 SSD 和 HDD ）。
...
== 将 StorageGRID 与 FabricPool 结合使用的其他注意事项
...
== 将阿里云对象存储与 FabricPool 结合使用的其他注意事项
...
== 将 AWS S3 与 FabricPool 结合使用的其他注意事项
...
* ONTAP 用于连接到 AWS S3 对象服务器的 LIF 必须位于 10 Gbps 端口上。
...
== 将 IBM 云对象存储与 FabricPool 结合使用的其他注意事项
...
== 将 Microsoft Azure Blob Storage 与 FabricPool 结合使用的其他注意事项
