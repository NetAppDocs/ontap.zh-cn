...
[role="lead"]
从 ONTAP 9.12.1 开始，ONTAP 包括一个预定义的基于角色的访问控制 (RBAC) 角色，称为 `admin-no-fsa`。该 `admin-no-fsa`角色授予管理员级别的权限，但阻止用户在 ONTAP CLI、ONTAP REST API 和 System Manager 中执行与 `files`端点（即文件系统分析）相关的操作。
