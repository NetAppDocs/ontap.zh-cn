...
启用容量分析后，ONTAP 将执行容量分析的初始化扫描。扫描将访问已启用容量分析的卷中所有文件的元数据。扫描期间不读取任何文件数据。从 ONTAP 9.14.1 开始，您可以使用 ONTAP REST API、System Manager 的 **Explorer** 选项卡或 `volume analytics show` ONTAP CLI 命令跟踪扫描进度。如果发生限制事件，ONTAP 将提供通知。
