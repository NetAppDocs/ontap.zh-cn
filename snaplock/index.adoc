...
SnapLock 有两个版本：SnapLock ：Compliance和SnapLock Enterprise。这两个版本均在具有所有存储类型的NetApp系统上运行。这种灵活性使您可以根据业务需求为SnapLock WORM存储购买类型和数量的存储。

SnapLock WORM存储使用NetApp Snapshot®技术、并可利用NetApp SnapMirror®复制软件和NetApp SnapVault®备份功能作为为数据提供备份恢复保护的基础技术。
...
如果以下情况适用、则不应使用SnapLock ：

* 您正在使用SAN LUN。SnapLock 卷不支持 SAN LUN 。虽然可以使用传统技术将 SAN LUN 移动到 SnapLock 卷上，但这不是受支持的操作，也不是涉及 SnapLock 卷上的 SAN LUN 的任何其他操作。
* 您正在使用SMTape。
* 您正在为阵列LUN创建合规性聚合。SnapLock 合规性聚合不支持阵列 LUN 。
* 您正在使用SyncMirror 选项创建合规性聚合。SnapLock 合规性聚合不支持 SyncMirror 丛。
...
SnapLock 不需要任何特殊硬件。SnapLock 是Data ONTAP 的一项许可功能、支持ONTAP 9的所有AFF 和FAS 系统均支持此功能。ONTAP Select 也支持此功能。SnapLock 不是纯软件解决方案 、而是集成的硬件和软件解决方案。这种区分对于SEC 17a-4等严格的WORM法规非常重要、因为这些法规要求集成硬件和软件解决方案。有关详细信息、请参阅SEC的解释：《Broker-Dealer Records的电子存储》。
