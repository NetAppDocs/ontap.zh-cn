...
使用 SnapLock，可以将文件和快照提交到 WORM 存储，并为受 WORM 保护的数据设置保留期。SnapLock WORM 存储使用 NetApp 快照技术，可以利用 SnapMirror 复制和 SnapVault 备份作为为数据提供备份恢复保护的基础技术。详细了解 WORM 存储：link:https://www.netapp.com/pdf.html?item=/media/6158-tr4526.pdf["使用NetApp SnapLock 的合规WORM存储—TR-4526"^]。
...
SnapLock是ONTAP的一项基于许可证的功能。一个许可证即可让您在严格的合规模式下使用SnapLock ，以满足 SEC 规则 17a-4(f) 等外部要求，以及在较为宽松的企业模式下使用 SnapLock，以满足内部强制规定的数字资产保护要求。SnapLock许可证是以下部分的一部分： link:../system-admin/manage-licenses-concept.html#licenses-included-with-ontap-one["ONTAP One"] 软件套件。
