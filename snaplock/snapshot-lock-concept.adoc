...
--
. 导航到*存储>卷*。
. 选择 ... image:icon_kabob.gif["菜单选项"] 然后选择*编辑>卷*。
...
. 选择*Settings*。
. 找到*Snapshot Policies*并选择 image:icon_arrow.gif["箭头"]。
. 在*添加Snapshot策略*窗口中，输入策略名称。
. 选择 ... image:icon_add.gif["添加"]。
...
. 导航到*存储>卷*并选择一个卷。
. 在卷详细信息页面中、选择* Snapshot副本*选项卡。
. 选择Snapshot副本、然后选择 image:icon_kabob.gif["菜单选项"]，然后选择*修改SnapLock到期时间*。您可以选择日历来选择保留到期日期和时间。
. 保存所做的更改。
. 在*卷> Snapshot副本*页面中、选择*显示/隐藏*并选择* SnapLock 到期时间*以显示* SnapLock 到期时间*列并验证是否已设置保留时间。
