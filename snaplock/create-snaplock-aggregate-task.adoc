...
summary: '您可以使用卷 `-snaplock-type`选项指定合规性或企业 SnapLock 卷类型。对于早于 ONTAP 9.10.1 的版本，您必须创建单独的 SnapLock 聚合。从 ONTAP 9.10.1 开始，SnapLock 和非 SnapLock 卷可以存在于相同的聚合上。因此，如果您使用的是 ONTAP 9.10.1，则不再需要创建单独的 SnapLock 聚合。' 
---
= 创建ONTAP SnapLock聚合
:allow-uri-read: 
:icons: font
