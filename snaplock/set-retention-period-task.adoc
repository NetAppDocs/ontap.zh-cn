...
IMPORTANT: 在ONTAP 9.10.1 之前的版本中，如果您在将合规模式文件提交到 WORM 状态之前没有明确设置保留时间，并且没有修改默认值，则该文件将保留 30 年。此更改_无法_撤销。同样，在ONTAP 9.10.1 及更高版本中，如果您在将企业模式文件提交到 WORM 状态之前没有明确设置保留时间，并且没有修改默认值，则该文件将保留 0 年，或者实际上根本不会保留。
...
====
有关高级用法，请参见 link:https://www.netapp.com/pdf.html?item=/media/6158-tr4526.pdf["使用 NetApp SnapLock 的合规 WORM 存储"^]。
