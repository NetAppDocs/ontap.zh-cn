...
[role="lead"]
您可以使用 `volume file fingerprint start` 和 `volume file fingerprint dump` 命令查看有关文件和卷的关键信息，包括文件类型（常规， WORM 或可附加 WORM ），卷到期日期等。

.步骤
. 生成文件指纹：
+
` * 卷文件指纹 start -vserver _svm_name_ -file _file_path_*`
...
----
+
命令将生成一个会话 ID ，您可以将其用作 `volume file fingerprint dump` 命令的输入。

+
[NOTE]
====
您可以使用带有会话 ID 的 `volume file fingerprint show` 命令来监控指纹操作的进度。在尝试显示指纹之前，请确保操作已完成。

====
. 显示文件的指纹：
+
` * 卷文件指纹转储 -session-id _session_ID_*`
