...
单击 * 保存 * 时，将在源 Storage VM 中创建一个新分段，并将其镜像到目标 VM 。
...
. 在源集群和目标集群上安装 CA 服务器证书：
+
.. 在源集群上，从 _destination_cluster 安装证书： `security certificate install -type server-ca -vserver _src_svm_ -ct-name _dest_server_certificate_`
.. 在目标集群上，从 _source_cluster 安装证书： `security certificate install -type server-ca -vserver _dest_svm_ -ct-name _src_server_certificate_`
+
如果您使用的证书由外部 CA 供应商签名，请在源和目标 SVM 上输入相同的证书。
