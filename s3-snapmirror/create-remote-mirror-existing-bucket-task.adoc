...
.. 在目标集群上，安装对 _source_S3 服务器证书签名的 CA 证书： `security certificate install -type server-ca -vserver _dest_admin_svm_ -cert-name _src_server_certificate_` + 如果您使用的是由外部 CA 供应商签名的证书，请在源和目标 SVM 上输入相同的证书。
+
