...
当您使用命令行界面 * ck * 保存 * 时，现有的 * 存储分段将镜像到目标 Storage VM 中的新存储分段。
...
. 在源集群和目标集群上安装 CA 证书：
+
.. 在 _source_cluster 上，从 _destination_cluster 安装 S3 服务器证书： `security certificate install -type server-ca -vserver _src_svm_ -ct-name _dest_server_certificate_`
.. 在 _destination cluster_ 上，从 _source_cluster 安装 S3 服务器证书： `security certificate install -type server-ca -vserver _dest_svm_ -ct-name _src_server_certificate_` + 如果您使用的是由外部 CA 供应商签名的证书，请在源和目标 SVM 上输入相同的证书。
+
