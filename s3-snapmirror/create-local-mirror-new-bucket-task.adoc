...
summary: 创建新的 S3 存储桶时，您可以立即将它们保护到同一集群上的 SnapMirror S3 目标。您可以将数据镜像到与源不同的 Storage VM 或同一个 Storage VM 中的存储分段。 
---
...
[role="lead"]
创建新的 S3 存储桶时，您可以立即将它们保护到同一集群上的 SnapMirror S3 目标。您可以将数据镜像到与源不同的 Storage VM 或同一个 Storage VM 中的存储分段。
