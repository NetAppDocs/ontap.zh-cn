...
. 确保用于签署NetApp服务器证书的 CA 证书（以及直至根 CA 的任何中间签名机构）采用隐私增强邮件 (PEM) 格式，并命名为“Ontap_CA.pem”。将其放置在杀毒软件连接器的安装目录中。在ONTAP系统上，将用于在“ONTAP”上为防病毒连接器签署客户端证书的 CA 证书（以及任何中间签名机构，直至根 CA）安装为“client-ca”类型的证书。
