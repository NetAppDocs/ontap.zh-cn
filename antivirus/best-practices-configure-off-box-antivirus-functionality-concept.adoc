...
* The `scan-queue-timeout`值应小于 `scanner-pool request-timeout`值。如果设置为更高的值，则对文件的访问可能会延迟，并可能最终超时。为避免这种情况，请将 `scan-queue-timeout`配置为比 `scanner-pool request-timeout`值少 5 秒。有关如何更改 `scan-queue-timeout`设置的说明，请参阅扫描引擎供应商的文档。 `scanner-pool timeout`可以通过在高级模式下使用以下命令并为 `request-timeout`参数提供适当的值来更改：
