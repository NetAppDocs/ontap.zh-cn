...
summary: 从 ONTAP 9.8 开始，您可以选择使用 IP 安全来保护您的网络流量。IPsec 是 ONTAP 提供的几种动态数据或传输中加密选项之一。在生产环境中使用 IPsec 之前，应做好配置 IPsec 的准备。 
---
= 了解 ONTAP 网络上的 IP 安全
:hardbreaks:
:allow-uri-read: 
...
如果使用的是 ONTAP 9.16.1 或更高版本，则可以选择将某些计算密集型操作（如加密和完整性检查）卸载到存储节点上安装的网络接口控制器（NIC）卡。与未启用 IPsec 加密的明文操作相比，卸载到 NIC 卡的 IPsec 加密操作对网络吞吐量性能的影响约为 5% 或更低。这可以显著提高受 IPsec 保护的网络流量的性能和吞吐量。
...
* X50131A（IO 模块，2PT，100/200GbE）
* X50131B（IO Module，2PT，200/400GbE，B）
* X60132A (4p、10G25G以太网控制器)


ONTAP 9.17.1 增加了对以下以太网卡的支持：

* X50135A (IO Module, 2PT, 100GbE, Encrypt)
* X60135A（2p，40G/100G以太网控制器）


X50131A、X50131B 和 X50135A 卡在以下平台上受支持：
