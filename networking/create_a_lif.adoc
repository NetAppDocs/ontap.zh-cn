...
+
iSCSI LIF故障转移会自动启用(故障转移策略设置为`sfo-partner-only`、自动还原值设置为`true`) 在新创建的iSCSI LIF上、如果指定的Storage VM中不存在iSCSI LIF、或者指定Storage VM中的所有现有iSCSI LIF已启用iSCSI LIF故障转移。

+
如果在升级到ONTAP 9.11.1或更高版本后、某个Storage VM中的现有iSCSI LIF尚未启用iSCSI LIF故障转移功能、而您在同一Storage VM中创建了新的iSCSI LIF、 新的iSCSI LIF采用与Storage VM中现有iSCSI LIF相同的故障转移策略(`d已标记为`)。
