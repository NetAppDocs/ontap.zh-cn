...
summary: 在创建NFS或SMB服务器之前、为SVM配置DNS服务。 
---
= 为ONTAP网络配置DNS服务
:hardbreaks:
:allow-uri-read: 
...
NOTE: 为避免不符合 DDNS 更新 RFC 规则的 SVM FQDN 配置失败，请使用符合 RFC 的 FQDN 名称。有关详细信息，请参见link:https://tools.ietf.org/html/rfc1123["RFC 1123"^]。
