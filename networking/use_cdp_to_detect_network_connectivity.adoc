...
summary: 在数据中心中，您可以使用邻居发现协议查看一对物理或虚拟系统与其网络接口之间的网络连接。ONTAP 支持两种邻居发现协议，即 Cisco Discovery Protocol 和 Link Layer Discovery Protocol。 
---
= 使用CDP检测ONTAP网络连接
:hardbreaks:
:allow-uri-read: 
...
此外，还必须在任何以太网交换机和路由器上启用 CDP，然后才能显示有关邻居设备的信息。
...
NOTE: 有时，当节点上的 LIF 发生变化时，接收设备端（例如以太网交换机）的 CDP 信息不会更新。如果遇到此类问题，应将节点的网络接口配置为关闭状态，然后再配置为启动状态。
...
一些供应商的交换机不支持 CDP。查看link:https://kb.netapp.com/onprem/ontap/da/NAS/ONTAP_device_discovery_shows_nodes_instead_of_the_switch["NetApp知识库： ONTAP设备发现显示节点而不是交换机"^]了解更多详情。
