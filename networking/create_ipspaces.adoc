...
summary: 为集群中的每个 SVM 创建 IPspace。 
---
= 为ONTAP网络创建IP空间
:hardbreaks:
:allow-uri-read: 
...
[NOTE]
====
从 ONTAP 9.19.1 开始，ONTAP 通过将两项丢包恢复功能（PRR 和 RACK）集成到 ONTAP TCP 堆栈中，提高了有损、高延迟 WAN 链路上的 TCP 数据传输性能。 link:improve-tcp-wan-performance.html["了解如何使用 PRR 和 RACK 提高 TCP WAN 性能"]
