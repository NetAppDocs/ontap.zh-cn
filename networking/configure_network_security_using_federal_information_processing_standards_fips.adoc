...
summary: 所有 SSL 连接均符合联邦信息处理标准 140-2。 
---
= 使用 FIPS 配置 ONTAP 网络安全
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
|===
在启用 FIPS 之前，必须重新配置现有 SSH 公钥帐户以使用受支持的密钥类型，否则管理员身份验证将失败。
...
. 当系统提示您继续时、输入 `y`
. 从 ONTAP 9.9.1 开始，不需要重新启动。如果运行的是 ONTAP 9.8 或更低版本，请单独手动重新启动集群中的每个节点。
...
您可以使用以下命令启用 SSLv3。如果您运行的是 ONTAP 9.9.1 或更高版本，则不会看到警告消息。
...
.步骤
. 如果您运行的是 ONTAP 9.8 或更低版本，请单独手动重新启动集群中的每个节点。
. 查看当前合规状态：
+
...
* link:../networking/configure-tls-offload.html["配置 TLS 硬件卸载"]
