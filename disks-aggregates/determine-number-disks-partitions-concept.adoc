...
[NOTE]
====
在 ONTAP 中，驱动器的可用空间小于驱动器的物理容量。您可以在中找到特定驱动器的可用空间以及每个RAID策略所需的最小磁盘数或磁盘分区数 https://["Hardware Universe"^]。
...
* 对于 Flash Pool 聚合，每个 HA 对至少应有两个备用磁盘。有关 Flash Pool 聚合支持的 RAID 策略的详细信息，请参见 https://["Hardware Universe"^]。
...
http://["NetApp 技术报告 3838 ：《存储子系统配置指南》"^]
