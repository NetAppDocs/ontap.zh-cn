...
[role="lead"]
策略类型为“vault”或“mirror-vault”的SnapMirror关系允许直接在SnapMirror关系的辅助卷上创建快照。这些快照将作为备份保留在目标位置。这些快照通常是为了长期保留而创建的，被称为长期保留快照。

您可以通过在SnapMirror策略规则中指定快照创建计划、快照名称前缀、 SnapMirror标签和保留计数来创建长期保留快照。无论源上的保留规则如何，此快照都会保留在SnapMirror目标卷上。

长期保留快照仅适用于FlexVol SnapMirror配置。您无法为FlexGroup SnapMirror配置创建长期保留快照。
