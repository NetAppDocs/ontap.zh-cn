...
| 应用程序创建的快照复制 | ONTAP 9.7 | 如果快照在  `snapshot create` 操作时使用 CLI 或 ONTAP API 使用适当的标签进行标记，则在停止应用程序后，SnapMirror 同步复制快照，无论是用户创建的快照还是使用外部脚本创建的快照。从 ONTAP 9.14.1 开始，应用程序创建的快照的复制标签要求不再存在。有关复制应用程序创建的快照的详细信息，请参见 link:https://kb.netapp.com/Advice_and_Troubleshooting/Data_Protection_and_Security/SnapMirror/How_to_replicate_application_created_snapshots_with_SnapMirror_Synchronous["NetApp知识库：如何使用SnapMirror同步复制应用程序创建的快照"^]。 


| 自主勒索软件保护 | ONTAP 9.19.1 |  
...
| NDMP还原 | ONTAP 9.13.1 | 源集群和目标集群都必须运行 ONTAP 9.13.1 或更高版本，才能对 SnapMirror 同步卷执行基于转储或 SMTape 的 NDMP 备份。不支持对 SnapMirror 活动同步或 SnapMirror 同步关系中的目标卷执行基于转储或 SMTape 的 NDMP 还原。 
...
| 快照管理操作的复制 | ONTAP 9.19.1 | 从 ONTAP 9.19.1 开始，快照管理操作 `snapshot delete`、 `snapshot rename` 和 `snapshot modify` 被复制到辅助集群。 


| 计划的 Snapshot 复制 | ONTAP 9.19.1 | 从 ONTAP 9.19.1 开始，您可以使用 SnapMirror 策略启用计划快照的复制。在早于 ONTAP 9.19.1 的版本中，不会复制使用快照策略创建的计划快照。 
...
| SVM 灾难恢复 | ONTAP 9.6  a| 
* SnapMirror 同步源也可以是 SVM 灾难恢复源，例如，以 SnapMirror 同步为一个分支、SVM 灾难恢复为另一个分支的扇出配置。
* 由于 SnapMirror 同步不支持级联数据保护源，因此 SnapMirror 同步源不能作为 SVM 灾难恢复目标。在目标集群中执行 SVM 灾难恢复反向重新同步之前，必须释放同步关系。
* SnapMirror 同步目标不能作为 SVM 灾难恢复源，因为 SVM 灾难恢复不支持 DP 卷的复制。同步源的反向重新同步将导致 SVM 灾难恢复排除目标集群中的 DP 卷。




| link:../snaplock/snapshot-lock-concept.html["防篡改快照"] | ONTAP 9.19.1 | 只能在创建快照期间指定到期时间。无法设置或延长现有快照的到期时间。 
...
* SnapCenter
* SnapLock 卷
* 转储或基于 SMTape 的 NDMP 还原到 SnapMirror 活动同步或 SnapMirror 同步关系中的目标卷
* 源卷的吞吐量下限（ QoS 最小值）
* Volume SnapRestore
* VVOL
...
从 ONTAP 9.19.1 开始，您可以在高级权限模式下使用 SnapMirror 策略参数 `-sync-snapshot-replication-mode`，将计划快照复制到 SnapMirror 目标。复制计划快照可在 SnapsMirror 同步关系中的主卷和二级卷之间实现对称性。

.前提条件
* 您必须是集群或 SVM 管理员
* 您必须处于高级权限模式
* 必须配置使用参数 `-sync-snapshot-replication-mode`并包含选项 `sched_created`的用户定义 SnapMirror 策略。
+
** 无法修改系统定义的策略。您必须创建新策略并重新建立关系。
** 用户定义的策略更改需要重新同步（静止后继续）才能对现有关系生效。
...
* `none`- 禁用复制非 SnapMirror 同步快照
* `application_created` - 复制应用程序创建的快照
* `sched_created` - 复制已计划的快照
* `sm_created` - 复制 SnapMirror 异步（扇出分支）创建的快照
* 以逗号分隔的类型列表： `application_created`、 `sched_created` 和  `sm_created`。


该参数仅在关系状态为 `InSync`时生效。未列出的快照类型仅存储在主卷上。

从 ONTAP 9.19.1 开始， `snapmirror initialize` 将所有快照复制到辅助集群，而  `snapmirror resync` 仅复制在最后一个通用快照之后创建的新快照。在重新同步操作期间，当关系状态为  `InSync` 时标记为仅主要的非复制快照不会复制到辅助集群，这些快照在翻转重新同步操作后会保留在新的辅助集群中。


NOTE: 在 SnapMirror 同步卷上创建快照可能会增加用户 I/O 延迟。建议您使用频率超过 30 分钟的快照计划。

.相关信息
* https://www.netapp.com/pdf.html?item=/media/17174-tr4733.pdf["NetApp技术报告4733：《SnapMirror同步配置和最佳实践》"^]
* link:https://docs.netapp.com/us-en/ontap-cli//volume-snapshot-create.html["卷 snapshot 创建"^]
* link:https://docs.netapp.com/us-en/ontap-cli/volume-snapshot-delete.html["volume snapshot delete"^]
* link:https://docs.netapp.com/us-en/ontap-cli/volume-snapshot-modify.html["卷 snapshot 修改"^]
* link:https://docs.netapp.com/us-en/ontap-cli/volume-snapshot-rename.html["卷 snapshot 重命名"^]
