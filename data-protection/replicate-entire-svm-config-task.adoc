...
* 从 ONTAP 9.19.1 开始，为 SVM DR 配置的 SVM 不支持启用 IPsec。如果使用 IPsec，则必须禁用它或选择其他 SVM 以配置 SVM DR 关系。如果使用 ONTAP 9.19.1 或更高版本并尝试在为 SVM DR 配置的 SVM 上启用 IPsec，您将收到错误。
...
. 从源集群中，使用 `vserver peer create`命令创建 SVM 对等关系。
+
