...
[role="lead"]
从 ONTAP 9.9.1 开始，当您想要将数据保护工作流转换到云时，可以使用 _SnapMirror cloud_。您可以使用对象存储作为长期数据保留和存档的替代存储库，而不是传统的磁带备份架构。您可以使用 Amazon S3、Azure Blob、Google Cloud Storage、StorageGRID 或 ONTAP S3 等提供商作为您的云对象存储。
...
+
** 当每个集群使用超过 6 个存储桶时，应为支持它的提供商使用虚拟托管式 URL 配置，包括 AWS S3、阿里云、StorageGrid、Google Cloud Storage、IBM Cloud 对象存储、ONTAP S3 和其他 S3 兼容提供商。建议使用此配置以启用可扩展对象存储寻址并避免潜在限制。
** 默认情况下，按需还原处于启用状态。
