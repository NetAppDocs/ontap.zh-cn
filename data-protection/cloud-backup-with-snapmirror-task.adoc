...
[role="lead"]
从 ONTAP 9.9.1 开始，当您想要将数据保护工作流转换到云时，可以使用 _SnapMirror cloud_。您可以使用对象存储作为长期数据保留和存档的替代存储库，而不是传统的磁带备份架构。您可以使用 Amazon S3、Alibaba Cloud、Azure Blob、Google Cloud Storage、IBM Cloud Object Storage、StorageGRID 或 ONTAP S3 等提供商作为您的云对象存储。

从 ONTAP 9.9.1 开始，您可以使用 System Manager 备份和还原您的数据。
