...
[NOTE]
====
您可能会发现保留一份命令输出副本以跟踪现有关系设置会很有帮助[`snapmirror show`。要详细了解ONTAP命令参考中的link：https://docs NetApp．ONTAP－SnapMirror-show.html[`snapmirror show`^)命令。
...
----
+
要详细了解SnapMirrorONTAP命令参考中的link：https://docs NetApp．ONTAP－CLI/Dell-quiesce.html[`snapmirror quiesce`^)命令。
...
----
+
要了解有关链接：https://docs ONTAP．ONTAP－SnapMirror-break.html^的更多信息，请参阅NetApp[`snapmirror-break`命令参考。
...
----
+
要详细了解SnapMirrorONTAP命令参考中的link：https://docs NetApp．ONTAP－CLI/Dell-delete.html[`snapmirror-delete`^)命令。
...
----
+
为了缩短重新同步时间、您可以使用 `-quick-resync`选项、但您应注意、存储效率节省的空间可能会丢失。要详细了解SnapMirrorONTAP命令参考中的link：https://docs NetApp．ONTAP－CLI/SE-resync.html#parameters.html[`snapmirror resync`^)命令。
