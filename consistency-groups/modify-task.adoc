...
* 从ONTAP 9。14.1开始、您可以使用SnapMirror异步将卷粒度快照复制到目标集群。使用SnapMirror异步扩展一致性组时、只有在SnapMirror策略为镜像全部或镜像AndVault时、才会在扩展一致性组后复制卷粒度快照。仅复制比基线一致性组Snapshot更新的卷粒度Snapshot。
