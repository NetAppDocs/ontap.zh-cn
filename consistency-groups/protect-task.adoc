...
* 调用快照创建时，可以将可选超时值设置为 5 到 120 秒之间。如果未提供超时值，则操作超时默认为 7 秒。在 ONTAP REST API 中，使用  `action_timeout` 参数设置超时值。在 ONTAP CLI 中，使用  `-timeout` 标志。


.步骤
您可以使用 ONTAP REST API 或从 ONTAP 9.14.1 开始的 ONTAP CLI 完成两阶段快照。System Manager 不支持此操作。


NOTE: 如果使用 ONTAP REST API 调用快照创建，则必须使用 ONTAP REST API 提交快照。如果使用 ONTAP CLI 调用快照创建，则必须使用 ONTAP CLI 提交快照。不支持混合使用方法。
