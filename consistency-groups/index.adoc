...
有关 ONTAP REST API 的更多信息，请参阅 https://docs.netapp.com/us-en/ontap-automation/reference/api_reference.html#access-a-copy-of-the-ontap-rest-api-reference-documentation["ONTAP REST API参考文档"^]。
...
您可以在 System Manager 一致性组菜单的**概述**选项卡中查看监控数据，也可以在 ONTAP REST API 中请求查看。从 ONTAP 9.14.1 开始，您可以使用 `consistency-group metrics show`命令通过 ONTAP CLI 查看一致性组指标。如需了解有关 `consistency-group metrics show`的更多信息，请参阅link:https://docs.netapp.com/us-en/ontap-cli/vserver-consistency-group-metrics-show.html["ONTAP 命令参考"^]。


NOTE: 在 ONTAP 9.13.1 中，您只能使用 ONTAP REST API 检索历史指标。从 ONTAP 9.14.1 开始，历史指标也可在 System Manager 中使用。
...
一致性组提供应用程序一致性保护，确保跨多个卷或 LIF 的数据一致性。创建一致性组的快照时，会在一致性组上建立"围栏"。在快照操作完成之前，围栏会启动 I/O 队列，以确保一致性组中所有实体的数据的时间点一致性。在快照创建操作（如计划快照策略或使用 System Manager 创建快照）期间，围栏可能会导致延迟暂时激增。在 link:https://kb.netapp.com/on-prem/ontap/DM/REST-API["ONTAP REST API文档"^] 和 link:https://docs.netapp.com/us-en/ontap-cli/["ONTAP 命令参考"^] 中了解有关 ONTAP REST API 和 ONTAP CLI 上下文的更多信息。
...
与放置在镜像聚合上的一致性组关联的卷和快照将复制到远程站点（站点 B）。站点 B 上的卷内容为一致性组提供写入顺序保证，使您能够在发生灾难时从站点 B 进行恢复。您可以在运行 ONTAP 9.11.1 或更高版本的集群上，通过一致性组使用 ONTAP REST API 和 System Manager 访问一致性组快照。从 ONTAP 9.14.1 开始，您还可以使用 ONTAP CLI 访问快照。
...
在 ONTAP REST API 中创建的一致性组快照可以通过 System Manager 的一致性组界面和一致性组 ONTAP REST API 端点进行管理。从 ONTAP 9.14.1 开始，一致性组快照也可以使用 ONTAP CLI 进行管理。
