...
summary: 您可以通过修改存储虚拟机的 NFS 服务器来配置 ONTAP 在其内部缓存中存储 NFS 用户凭据的时间长度。这使您能够缓解与名称服务器上的高负载或影响 NFS 用户访问的凭据更改相关的某些问题。 
---
...
TTL 以毫秒为单位。从 ONTAP 9.17.1 及更高版本开始，默认值为 1 小时（3,600,000 毫秒）。在 ONTAP 9.16.1 及更早版本中，默认值为 24 小时（86,400,000 毫秒）。此值的允许范围为 1 分钟（60,000 毫秒）到 7 天（604,800,000 毫秒）。
