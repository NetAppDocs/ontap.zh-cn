...
ONTAP 9.9.1 及更高版本默认启用 NFSv4.2。在ONTAP 9.8 中，需要通过指定以下参数手动启用 v4.2： `-v4.2`选项并将其设置为 `enabled`在存储虚拟机 (SVM) 上创建 NFS 服务器时。启用 NFSv4.1 还允许客户端在挂载为 v4.2 时使用 NFSv4.1 的功能。
