...
====
* 客户端必须支持 RPC-with-TLS over TCP，必须配置为在挂载时请求 TLS，并且必须信任对 NFS 服务器证书进行签名的证书颁发机构。此处未记录客户端配置步骤，因为这些步骤因客户端而异。有关 Linux 客户端的端到端演练，请参阅 link:https://docs.netapp.com/us-en/ontap-technical-reports/nfs-tls/nfs-tls-configure.html["配置 NFS over TLS"]。
