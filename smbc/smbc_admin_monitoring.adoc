...
[role="lead"]
您可以监控以下SnapMirror业务连续性(SM-BC)操作、以确保SM-BC配置的运行状况：
...
在正常操作期间、ONTAP调解器状态应为已连接。如果处于任何其他状态，则可能表示出现错误情况。您可以查看 link:https://docs.netapp.com/us-en/ontap-ems-9131/sm-mediator-events.html["事件管理系统(EMS)消息"^] 确定错误并采取适当的更正操作。
