...
+
该访问应该会成功。如果没有，请确保所有防火墙都已正确配置以允许 HTTPS 和 DNS 流量，并且代理服务器已正确配置。有关为 support.netapp.com 配置静态名称解析的更多信息，请参阅link:https://kb.netapp.com/Advice_and_Troubleshooting/Data_Storage_Software/ONTAP_OS/How_would_a_HOST_entry_be_added_in_ONTAP_for_support.netapp.com%3F["NetApp知识库：如何在ONTAP中为 support.netapp.com 添加 HOST 条目？"^]
