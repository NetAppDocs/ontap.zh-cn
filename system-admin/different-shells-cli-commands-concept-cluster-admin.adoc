...
可以使用以下方式访问集群 shell 中支持的 Nodeshell 选项： `vserver options clustershell` 命令。要查看这些选项，您可以使用以下命令查询 clustershell CLI： `vserver options -vserver _nodename_or_clustername_ -option-name ?`
