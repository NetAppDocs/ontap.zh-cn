...
summary: 从 ONTAP 9.3 开始，您可以为 Web 服务配置安全断言标记语言身份验证。 
---
...
NOTE: 某些 URL 包含问号 (?) 字符。问号激活 ONTAP 命令行活动帮助。要输入带有问号的 URL，需要先使用命令 `set -active-help false`禁用活动帮助。稍后可以使用命令 `set -active-help true`重新启用活动帮助。如需了解更多信息，请参阅link:https://docs.netapp.com/us-en/ontap-cli/set.html["ONTAP 命令参考"^]。
...
NOTE:  `user_name` 值区分大小写。除非使用的是 Entra ID，否则仅包括用户名，不包括域的任何部分。如果使用的是 Entra ID，则可以使用域创建用户名，例如 _user_name@domain.com_。
