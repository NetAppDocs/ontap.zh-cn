...
* 从 ONTAP 9.19.1 开始，管理员可以使用 link:https://docs.netapp.com/us-en/ontap-cli/security-ssh-publickey-show.html["security ssh publickey show -vserver  -host-key-algorithm  命令"^] 查看 ONTAP 主机密钥指纹。这使管理员能够验证此指纹是否与 ONTAP 中存储的指纹匹配。
