...
* 从 ONTAP 9.13.1 开始，您可以选择将证书验证添加到 SSH 身份验证过程，以增强登录安全性。为此，link:../authentication/manage-ssh-public-keys-and-certificates.html["将X.509证书与公共密钥相关联"]账户使用。如果同时使用 SSH 公钥和 X.509 证书通过 SSH 登录，ONTAP 会在使用 SSH 公钥进行身份验证之前检查 X.509 证书的有效性。如果证书过期或被吊销，SSH 登录将被拒绝，SSH 公钥将自动禁用。
* 从 ONTAP 9.14.1 开始，ONTAP 管理员可以link:../authentication/configure-cisco-duo-mfa-task.html["在SSH身份验证过程中添加Cisco Duo双重身份验证"]增强登录安全性。启用 Cisco Duo 身份验证后首次登录时，用户需要注册一个设备作为 SSH 会话的身份验证器。
* 从 ONTAP 9.15.1 开始，管理员可以link:../authentication/dynamic-authorization-overview.html["配置动态授权"]根据用户的信任度向 SSH 用户提供额外的自适应身份验证。
* 从 ONTAP 9.19.1 开始，管理员可以使用 link:https://docs.netapp.com/us-en/ontap-cli/search.html?q=security+ssh+publickey+show["security ssh publickey show -vserver  -host-key-algorithm  命令"^] 查看 ONTAP 主机密钥指纹。这使管理员能够验证此指纹是否与 ONTAP 中存储的指纹匹配。
