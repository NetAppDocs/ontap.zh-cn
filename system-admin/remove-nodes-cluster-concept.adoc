...
----
+
主节点是包含 `mgmt`、 `vldb`、 `vifmgr`、 `bcomd`和 `crs`等进程的节点。

. 如果您要删除的节点是当前主节点，请启用集群中的另一个节点作为主节点：
+
.. 使当前主节点不符合参与集群的资格：
+
[source, cli]
...
----
+
在恢复资格之前，节点将被标记为运行状况不良。当主节点变得不符合条件时，集群仲裁将从其余节点中选出一个新的主节点。
...
如果选择配对节点作为新的主节点，则需要在使其不符合条件之前验证它是否持有 epsilon。如果配对节点持有 epsilon，则需要在使其不符合条件之前，将 epsilon 移动到集群中保留的其他节点。在重复这些步骤以删除配对节点时，请执行此操作。
...
+
如果节点是仲裁主节点，则集群将短暂失去仲裁，然后恢复仲裁。此仲裁损失是暂时的，不会影响任何数据操作。
