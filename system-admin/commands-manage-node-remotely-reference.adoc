...
了解有关 SP 和 BMC 可用的 `help`命令选项的更多信息，请参见link:online-help-at-sp-bmc-cli-task.html["在ONTAP SP或BMC命令行界面中查看帮助信息"]。

在link:bmc-cli-commands-reference.html["ONTAP基板管理控制器支持的命令行界面命令"]中详细了解相应的 ONTAP BMC CLI。
