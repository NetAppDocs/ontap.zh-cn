...
summary: 许可证是一个或多个软件授权的记录。从 ONTAP 9.10.1 开始，所有许可证都以 NetApp 许可证文件的形式交付，这是一个启用多个功能的单个文件。从 2023 年 5 月开始，所有 AFF 系统和 FAS 系统都与 ONTAP One 软件套件或 ONTAP Base 软件套件一起销售，从 2023 年 6 月开始，所有 ASA 系统都与 ONTAP One for SAN 一起销售。 
---
= ONTAP许可概述
:allow-uri-read: 
:icons: font
...
ONTAP One 包含所有可用的许可功能。它包含前 Core 捆绑包、Data Protection 捆绑包、Security and Compliance 捆绑包、Hybrid Cloud 捆绑包和 Encryption 捆绑包内容的组合。加密在受限国家/地区不可用。
...
从 2023 年 6 月开始，使用 28 个字符的许可证密钥的 ONTAP 系统也可以link:https://kb.netapp.com/onprem/ontap/os/How_to_get_an_ONTAP_One_license_when_the_system_has_28_character_keys["升级到ONTAP One或ONTAP基本兼容包"^]。
