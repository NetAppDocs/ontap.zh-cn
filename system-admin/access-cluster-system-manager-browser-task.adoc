...
summary: 如果您更喜欢使用图形界面而不是命令行界面来访问和管理集群，可以通过使用 System Manager 来实现此目的，System Manager 作为 Web 服务包含在 ONTAP 中，默认情况下处于启用状态，可通过浏览器进行访问。 
---
...
[NOTE]
====
从ONTAP 9.12.1 开始，系统管理器与NetApp控制台完全集成。

使用控制台，您可以从单个控制平面管理混合多云基础设施，同时保留熟悉的系统管理器仪表板。

看link:../concepts/sysmgr-integration-console-concept.html["System Manager 与NetApp Console 集成"] 。
