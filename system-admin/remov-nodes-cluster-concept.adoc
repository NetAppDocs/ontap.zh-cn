...
+
.. 如果节点持有epsilon、请将其资格更改为false。
+
`*cluster modify -node <node_name> -eligibility false*`



. 如果要删除的节点是当前主节点、请通过将主节点的集群资格更改为来选择集群中的另一个节点作为主节点 `false`：
+
`*cluster modify –eligibility false*`

+
主节点是指包含 " `mGMT` " ， "`VLDB` " ， "`vifmgr` " ， "`bcomd` " 和 "`crs` " 等进程的节点。。 `cluster ring show` 高级命令可显示当前主节点。
