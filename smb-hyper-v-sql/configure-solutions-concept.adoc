...
summary: 您可以使用持续可用的 SMB 3.0 及更高版本文件共享，将 Hyper-V 虚拟机文件或 SQL Server 系统数据库和用户数据库存储在驻留在 SVM 中的卷上，同时为计划内和计划外事件提供无中断操作。 
---
...
* NetApp Backup and Recovery 保护托管在 CIFS 共享上的独立 Hyper-V 虚拟机、FCI 群集实例和 System Center Virtual Machine Manager (SCVMM) 调配的虚拟机。
+
有关使用 NetApp Backup and Recovery 保护 Hyper-V 工作负载的信息，请参见 https://docs.netapp.com/us-en/data-services-backup-recovery/br-use-hyperv-protect-overview.html["保护 Hyper-V 工作负载概述"]。
