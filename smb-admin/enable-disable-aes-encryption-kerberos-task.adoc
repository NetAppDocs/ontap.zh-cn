...
summary: 要利用基于 Kerberos 的通信的最强安全性，可以在 SMB 服务器上启用 AES-256 和 AES-128 加密。如果不希望 SMB 服务器为与 Active Directory KDC 的基于 Kerberos 的通信选择 AES 加密类型，则可以禁用 AES 加密。默认情况下，禁用 AES 加密。 
---
...
修改  `-advertised-enc-types` 参数时，指定的加密类型将替换任何先前配置的类型。所有先前的加密类型设置均已删除。

为了增强安全性，每次修改 AES 安全选项时，Storage Virtual Machine (SVM) 都会在 AD 中更改其计算机帐户密码。更改密码可能需要包含计算机帐户的组织单位 (OU) 的管理 AD 凭据。只有当 ONTAP 必须在 Active Directory 中重置计算机帐户密码，并且您的 AD 配置需要包含计算机帐户的组织单位的管理凭据时，才会显示凭据提示。如果您的 AD 已同步并启用了自动重置，则不会显示提示。
