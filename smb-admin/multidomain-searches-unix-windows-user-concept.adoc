...
从 ONTAP 9.13.1 开始，当 ONTAP 在多域名称映射操作期间搜索受信任域时，它会对这些受信任域执行站点发现，并使用发现的站点进行域控制器查找。此行为有助于确保 DNS SRV 查找使用受信任域域控制器的正确站点信息。如果受信任域站点发现失败，ONTAP 将使用 SMB 服务器的默认站点。
