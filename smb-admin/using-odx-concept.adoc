...
+
ODX 复制卸载传递支持从 Hyper-V 来宾操作系统启动的复制操作。来宾操作系统必须支持 ODX 并使用由支持 ODX 的 SMB 或 SAN 存储支持的 SCSI 虚拟磁盘。IDE 虚拟磁盘不支持 ODX 传递。源和目标存储必须在同一集群上。

+
有关详细信息，请参见link:use-cases-odx-concept.html["ONTAP SMB 服务器上 ODX 的用例"]。
