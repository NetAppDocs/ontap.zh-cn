...
summary: 从 ONTAP 9.3 开始，您可以修改发现域控制器的默认流程。这使您能够将发现限制在您的站点或首选 DC 池中，从而可以根据环境提高性能。 
---
= 管理ONTAP SMB域控制器发现
:allow-uri-read: 
:icons: font
...
* 仅发现本地站点中的 DC 。
+
可以将 SMB 服务器的 `default-site`参数定义为将此模式与未分配给站点和服务中的站点的 LIF 一起使用。从 ONTAP 9.13.1 开始，如果 ONTAP 无法发现受信任域的站点信息，默认站点也可用作回退站点。
