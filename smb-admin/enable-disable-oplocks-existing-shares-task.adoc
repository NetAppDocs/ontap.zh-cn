...
通过修改现有共享在共享上启用机会锁
 a| 
输入以下命令： `vserver share properties add -vserver _vserver_name_ -share-name share_name -share-properties oplocks`
...
通过修改现有共享禁用共享上的机会锁
 a| 
输入以下命令： `vserver share properties remove -vserver _vserver_name_ -share-name _share_name_ -share-properties oplocks`
