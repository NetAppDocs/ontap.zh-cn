...
[NOTE]
====
Microsoft 不再建议使用 `Digitally sign communications (if client agrees)`或 `Digitally sign communications (if server agrees)`组策略设置。Microsoft 也不再建议使用 `EnableSecuritySignature`注册表设置。这些选项仅影响 SMB 1 行为，可以由 `Digitally sign communications (always)`组策略设置或 `RequireSecuritySignature`注册表设置替换。
