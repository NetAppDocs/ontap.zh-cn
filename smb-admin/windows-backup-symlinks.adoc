...
summary: 当在 Windows 上运行的备份应用程序遇到 Unix 样式的符号链接时，将跟随该链接并备份数据。从 ONTAP 9.15.1 开始，您可以选择备份符号链接而不是数据。ONTAP FlexGroup 卷和 FlexVols 完全支持此功能。 
---
...
除了 ONTAP CLI 之外，您还可以使用 ONTAP REST API 管理此功能。有关详细信息，请参见 https://docs.netapp.com/us-en/ontap-automation/whats-new.html["ONTAP REST API和自动化的新增功能"^]。确定 ONTAP 如何处理 Unix 样式符号链接的配置必须为每个 SVM 单独执行。
