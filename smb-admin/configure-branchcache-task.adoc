...
summary: 您可以为单个共享或所有 SMB 共享配置 BranchCache。如果选择每共享配置，则必须在应提供缓存的每个 SMB 共享上启用 BranchCache。 
---
...
[role="lead"]
要使用 BranchCache，必须配置客户端，并在需要时配置内容将被缓存的分支机构中的托管缓存服务器。您可以为单个共享或所有 SMB 共享配置 BranchCache。如果您选择每共享配置，则必须在应提供缓存的每个 SMB 共享上启用 BranchCache。您还可以设置 BranchCache 为在所有 SMB 共享上自动启用缓存。
...
----
.. 检查已配置的 SVM SMB 设置，以确定是否已启用所有需要的 SMB 版本：
+
[source, cli]
----
