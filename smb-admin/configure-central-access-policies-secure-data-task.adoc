...
summary: 您必须采取几个步骤来使用中央访问策略保护对 CIFS 服务器上数据的访问，包括在 CIFS 服务器上启用动态访问控制，在 Active Directory 中配置中央访问策略，使用 GPO 将中央访问策略应用于 Active Directory 容器，以及在 CIFS 服务器上启用 GPO。 
---
