...
summary: 有四种不同的安全样式 - UNIX、NTFS、混合和统一。每种安全样式对数据的权限处理方式都有不同的影响。您必须了解不同的效果，以确保为您的目的选择适当的安全样式。 
---
= 了解ONTAP SMB安全模式及其影响
:allow-uri-read: 
:icons: font
...
| NTFS | SMB | NTFS ACL | NTFS | NFS 和 SMB 
...
* NTFS ACL
 ...
 a| 
* UNIX（用于 NFSv3 模式位和 NFSv4.x ACL）
* NTFS（用于 NTFS ACL）
...
| 统一：
(仅限无限卷、在ONTAP 9.4及更早版本中。) | NFS 或 SMB  a| 
* NFSv3 模式位
* NFSv4.1 ACL
* NTFS ACL

 a| 
* UNIX（用于 NFSv3 模式位和 NFSv4.1 ACL）
* NTFS（用于 NTFS ACL）
...
NOTE: 可以在 FlexVol 卷（根卷或数据卷）和 qtree 上设置安全样式。安全样式可以在创建时手动设置、自动继承或稍后更改。== 决定在 ONTAP SVM 上使用哪些 SMB 安全样式
