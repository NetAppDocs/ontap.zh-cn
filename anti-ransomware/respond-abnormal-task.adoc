...
.. 执行以下操作之一：
+
*** 对于文件类型警告，请选择 *Update and Clear Suspected File Types* 。
*** 对于熵尖峰，选择*标记为假阳性*。
+
...
NOTE: 从ONTAP 9.13.1 开始，如果您使用 MAV 来保护 ARP 设置，清除可疑项操作会提示您获得一个或多个其他管理员的批准。link:../multi-admin-verify/request-operation-task.html["必须获得所有管理员的批准"]与 MAV 审批小组相关，否则操作将会失败。
...
. 从 9.18.1 版本开始，您可以确定以下状态： `clear-suspect`手术：
+
[source, cli]
----
