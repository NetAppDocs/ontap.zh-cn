...
* 在 ONTAP 9.19.1 中，ARP 配置状态和 ARP 快照不会作为 SnapMirror 同步或 SnapMirror 活动同步的一部分复制到辅助卷。
...
=== SnapMirror 升级到 ONTAP 9.19.1 后的关系

如果在升级后对属于 SnapMirror 同步或 SnapMirror 活动同步关系的卷启用 ARP，则该关系会短暂移至不同步状态。这种情况是暂时的，关系应自动重新同步，无需干预。
