...
从 ONTAP 9.18.1 开始，在群集升级或新安装后的 12 小时宽限期之后，默认情况下在 link:use-cases-restrictions-concept.html#supported-configurations["支持的系统"] 的群集级别的所有新卷上都启用 ARP。如果在群集级别禁用 ARP 的自动默认启用，您仍然可以选择在 SVM 级别的所有新卷上默认手动启用 ARP。
