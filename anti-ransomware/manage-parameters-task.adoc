...
NOTE: 要减少大量误报警报，请转到*存储 > 卷 > 安全 > 配置工作负载特性*并禁用*监控新文件类型*。默认情况下，在 ONTAP 9.14.1 P7、9.15.1 P1、9.16.1 及更高版本中禁用此设置。从 ONTAP 9.18.1 P2 开始，System Manager 不再显示 ARP 工作负载特性，包括基线和浪涌统计。使用 ONTAP CLI 和 REST API 选项进行高级分析的浪涌统计数据仍然可用。
