...
一个比喻是车辆的安全特性。您不希望依赖安全带等单一功能来在发生事故时为您提供全面保护。安全袋，防抱死制动器和前向碰撞警告都是额外的安全功能，可以带来更好的结果。应以相同方式查看勒索软件保护。

虽然ONTAP 包含FPolicy、Snapshot副本、SnapLock 和Active IQ 数字顾问等功能、可帮助防止勒索软件、但以下信息重点介绍具有机器学习功能的ONTAP ARP即装即用功能。

要详细了解 ONTAP 的其他反勒索软件功能，请参见： https://["TR-4572 ：《 NetApp 解决方案 for 勒索软件》。"^]
