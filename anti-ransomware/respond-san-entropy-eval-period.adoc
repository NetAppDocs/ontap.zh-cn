...
[role="lead"]
从ONTAP 9.17.1 开始，ARP 需要一段评估期来确定 SAN 卷工作负载的熵级别是否适合勒索软件防护。在 SAN 卷上启用 ARP 后，ARP/AI 会在评估期间主动监控和保护该卷，同时确定最佳加密阈值。在评估期间，可以使用保守阈值进行检测和发出警报，同时建立基线阈值。会区分评估后的 SAN 卷中适用和不适用的工作负载，如果确定工作负载适合防护，则会根据评估期统计数据自动设置加密阈值。
