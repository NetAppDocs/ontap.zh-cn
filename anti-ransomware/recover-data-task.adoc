...
* 故障转移期间不会复制 ARP 启用状态。故障转移后，您需要在恢复卷上 link:enable-task.html["再次启用 ARP"]。
...
+
如果卷是 ONTAP 9.19.1 RC 中的 SnapMirror 同步或 SnapMirror 活动同步 SAN 关系的一部分，并且您计划执行卷级还原，请按照 SnapMirror 同步或 SnapMirror 活动同步文档静止或 link:../data-protection/make-destination-volume-writeable-task.html["断开保护关系"] 在开始还原之前，然后在还原完成后重新建立保护。
...
NOTE: 如果使用以下 `volume snapshot restore`命令，则只需在从早期快照还原之前释放反勒索软件 SnapLock。如果要使用 FlexClone、单个文件 SnapRestore 或其他方法还原数据，则无需执行此操作。
...
. 如果卷是 ONTAP 9.19.1 RC 中的 SnapMirror 同步或 SnapMirror 活动同步 SAN 关系的一部分，并且您打算使用 `volume snapshot restore`执行卷级还原，请在运行还原操作之前根据 SnapMirror 同步或 SnapMirror 活动同步文档进行静止或link:../data-protection/make-destination-volume-writeable-task.html["中断 SnapMirror 同步或 SnapMirror 活动同步关系"]。
. 列出卷中的快照：
+
[source, cli]
----
