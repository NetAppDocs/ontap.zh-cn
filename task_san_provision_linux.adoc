...
LUN 在 Linux 中显示为 SCSI 磁盘设备。


NOTE: 在创建 LUN 期间，始终会启用非对称逻辑单元访问（ ALUA ）。您不能更改 ALUA 设置。

您必须具有 FC 许可证，并且必须启用该许可证。如果未启用FC许可证、则LIF和SVM显示为联机、但运行状态为已关闭。要使 LIF 和 SVM 正常运行，必须启用 FC 服务。要托管启动程序，必须对 SVM 中的所有 FC LIF 使用单个启动程序分区。

您需要了解 Linux 服务器的启动程序标识符（ FC WWPN 或 iSCSI iqn ）。
...
.System Manager
--
使用ONTAP 系统管理器(9.7及更高版本)中的FC或iSCSI SAN协议创建LUN以为Linux服务器提供存储。
...
.命令行界面
--
使用FC或iSCSI SAN协议和ONTAP 命令行界面创建LUN以为Linux服务器提供存储。
