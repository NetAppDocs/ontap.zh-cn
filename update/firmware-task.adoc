...
summary: 您可以将固件更新应用于集群中受支持的设备，例如磁盘、磁盘架、SP 或 BMC。 
---
= 手动更新固件
:hardbreaks:
:toclevels: 1
...
[role="lead"]
从 ONTAP 9.9.1 开始，如果您已注册 link:https://docs.netapp.com/us-en/active-iq/["Digital Advisor"^]，则可以在 System Manager 中收到警报，通知您何时集群上受支持设备（如磁盘、磁盘架、服务处理器 (SP) 或基板管理控制器 (BMC)）的固件更新处于挂起状态。

如果您正在运行 ONTAP 9.8 或未在 Digital Advisor 中注册，请导航到 NetApp 支持站点以下载固件更新。
...
.步骤
根据您的 ONTAP 版本以及您是否已在 Digital Advisor 中注册，按照相应的步骤进行操作。
