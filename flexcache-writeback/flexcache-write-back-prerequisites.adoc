...
* 源端 *_必须_* 运行 ONTAP 9.15.1 或更高版本；但是，ONTAP 9.15.1 没有 FlexCache 写回所需的全部修复和改进，不建议用于生产工作负载。ONTAP 9.17.1P1 中引入了针对 FlexCache 写回的重大增强功能。*_强烈_* 建议您在源端集群和缓存集群上运行 9.17.1P1 之后的当前推荐版本。如果您无法运行 ONTAP 9.17.1，则下一个建议版本是 9.16.1 的最新 P 版本。
+

NOTE: *_强烈_*建议您在环境中实施 FlexCache 回写之前先查看 link:flexcache-write-back-guidelines.html["FlexCache回写准则"]。

* 任何需要在回写模式*_Must_*下运行的缓存集群都运行ONTAP 9.15.1.或更高版本。
* 任何不需要在写回模式下运行的缓存集群都可以运行支持 FlexCache 卷的任何常规 ONTAP 版本。
