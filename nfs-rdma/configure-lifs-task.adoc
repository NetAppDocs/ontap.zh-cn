...
NOTE: 每个 RDMA 端口最多可包含 127 个 LIF。当您添加更多 LIF 时，或当 LIF 故障切换到超过该限制的端口时，新添加的 LIF 或故障切换的 LIF 无法提供 RDMA 流量。
...
. 选择Storage VM和主节点。分配*名称*、*IP地址*和*子网掩码*。
...
** The `-rdma-protocols` 参数接受一个默认为空的列表。当 `roce` 作为值添加时，只能在支持 RoCE 卸载的端口上配置 LIF，这会影响 LIF 迁移和故障转移。
