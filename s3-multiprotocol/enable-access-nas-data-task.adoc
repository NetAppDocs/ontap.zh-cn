...
+
.. 单击 *存储 > 存储 VM*，选择一个支持 NAS 的存储 VM，单击 *设置*，然后在 *协议 > S3* 下单击 S3 下的 image:icon_gear.gif["操作图标"]。
...
. 在配置了NAS协议的Storage VM上启用S3。
+
.. 单击 *Cluster > Storage VMs*，选择一个已准备好 NAS 的存储 VM，单击 *Settings*，然后在 *Protocols > S3* 下单击 image:icon_gear.gif["操作图标"]。
.. 选择证书类型。无论选择系统生成的证书还是您自己的证书之一，客户端访问都需要此证书。
.. 输入网络接口。
...
+
** 不会再显示此机密密钥。
** 如果再次需要证书信息：单击 *Cluster > Storage VMs*，选择 Storage VM，然后单击 *Settings*。
