...
S3 NAS的"分段"只是S3客户端的NAS数据映射、而不是标准S3分段。因此、无需使用NetApp S3 SnapMirror功能来保护S3 NAS分段。相反、您可以使用SVM DR复制包含S3 NAS分段的源SVM、此SVM DR是与目标SVM的标准SnapMirror数据保护关系。SVM DR是S3多协议唯一支持的SnapMirror复制方法。不支持SnapMirror同步。
