...
例如，您可能希望设置一个存储桶策略以向一大组用户授予只读数据访问权限，并设置另一个存储桶策略以允许受限组对该数据的一个子集执行操作。

从 ONTAP 9.18.1 开始，您可以创建 NAS 存储桶，使应用程序能够使用 S3 协议访问 FlexCache 卷上的数据。集群中的所有节点都必须运行 ONTAP 9.18.1 或更高版本。在使用 S3 协议访问 FlexCache 卷之前，必须将  `-is-s3-enabled` 选项设置为  `true` link:../flexcache/create-volume-task.html["在 FlexCache 卷上"]。参数默认设置为  `false`。
...
.System Manager
--
在启用 NAS 的存储虚拟机上添加一个新的 S3 NAS 存储桶。
