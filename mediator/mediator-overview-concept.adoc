...
ONTAP Mediator 依靠 SCST 提供 iSCSI（请参阅 http://scst.sourceforge.net/index.html[]）。此包是专门为内核在安装过程中编译的内核模块。对内核的任何更新都可能需要重新安装 SCST。或者，卸载然后重新安装 ONTAP Mediator，然后重新配置 ONTAP 关系。


NOTE: 对服务器操作系统内核的任何更新都应与 ONTAP 中的维护窗口进行协调。
