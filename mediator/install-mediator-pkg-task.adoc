...
summary: 要安装 ONTAP Mediator，必须获取安装包并在主机上运行安装程序。 
---
= 安装 ONTAP 调解器安装包
:allow-uri-read: 
:icons: font
...
[role="lead"]
要安装 ONTAP Mediator，必须获取安装包并在主机上运行安装程序。
...
NOTE: 要在安装过程中跳过签名检查，请使用以下命令： `./ontap-mediator-1.11.0/ontap-mediator-1.11.0 -y --skip-code-signature-check`
