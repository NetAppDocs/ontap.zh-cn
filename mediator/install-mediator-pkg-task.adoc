...
[CAUTION]
====
* 在 MetroCluster IP 配置中，当使用 HTTPS 协议配置 ONTAP Mediator 时，单个 ONTAP Mediator 实例只能监控一个 MetroCluster IP 配置。不支持同时监控多个 MetroCluster IP 配置。
...
** 如果您计划仅使用 ONTAP Mediator 进行 SnapMirror 活动同步，或者您计划使用 HTTPS 配置 ONTAP Mediator 以监控每个 ONTAP Mediator 实例的单个 MetroCluster IP 配置，请进行响应 `No`。
+
或者，您可以在 `./ontap-mediator-1.12.0/ontap-mediator-1.12.0 -y` 命令中使用 `--https-only` 选项来仅指定 HTTPS。
...
[NOTE]
====
要在安装过程中跳过签名检查，请使用以下命令： `./ontap-mediator-1.12.0/ontap-mediator-1.12.0 -y --skip-code-signature-check`
