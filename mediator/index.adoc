...
* 当您升级到包含 SCST 包的新 ONTAP Mediator 版本时，安装程序会自动升级到推荐的 SCST 版本，除非有更高版本可用。有关手动安装更高 SCST 版本的说明，请参见 link:manage-task.html["管理ONTAP调解器"]。有关支持的版本，请参见 link:whats-new-concept.html#scst-support-matrix["SCST支持列表"]。
...
* 在 ONTAP Mediator 中，SCST 包用于为 MetroCluster IP 配置中的通信提供 iSCSI。SCST 包是在安装过程中编译的内核模块。从 ONTAP 9.19.1 和 ONTAP Mediator 1.12 开始，您可以配置使用 HTTPS 而不是 iSCSI 监控 MetroCluster IP 配置的 ONTAP Mediator。
...
NOTE: 对于使用 HTTPS 进行通信的 SnapMirror 活动同步或 MetroCluster IP ONTAP Mediator，ONTAP 不需要启用或连接端口 3260。
...
NOTE: 如果您安装了仅支持 HTTPS 的 ONTAP Mediator 1.12 或更高版本，则未安装 SCST 包，因此内核兼容性要求不适用。
