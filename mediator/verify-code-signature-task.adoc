...
[NOTE]
====
* 从 2025 年 6 月 15 日起，您无法安装或升级到 ONTAP Mediator 1.9 和 1.8，因为代码签名验证证书已过期。相反，请安装或升级到 ONTAP Mediator 1.11 或 1.10。
...
----
. 使用关联证书验证  `ontap-mediator-1.11.0.sig.tsr` 和  `ontap-mediator-1.11.0.tsr` 文件：
+
[role="tabbed-block"]
====
.OpenSSL 3.x
