...
+
PKI 机构验证请求并签署 `.csr`，生成证书 `intermediate.crt`。此外，您还需要获得 `root_ca.crt`签署的证书 `intermediate.crt`来自 PKI 机构的证书。

+

NOTE: 对于使用配置了 HTTPS 的 ONTAP Mediator 的 SnapMirror 业务连续性 (SM-BC) 集群和 MetroCluster IP 配置，必须将 `intermediate.crt`和 `root_ca.crt`证书添加到 ONTAP 集群。请参阅link:../snapmirror-active-sync/mediator-install-task.html["为 SnapMirror 主动同步配置 ONTAP 调解器和集群"]和link:https://docs.netapp.com/us-en/ontap-metrocluster/install-ip/task_configuring_the_ontap_mediator_service_from_a_metrocluster_ip_configuration.html["为 MetroCluster IP 配置配置 ONTAP Mediator"^]。
...
+
** 这 `ssl_keyfile`值是关键路径 `ontap_mediator_server.crt`文件，即 `ontap_mediator_server.key`。
...
NOTE: 对于SnapMirror业务连续性 (SM-BC) 集群，您必须添加 `intermediate.crt`和 `root_ca.crt`证书到ONTAP集群。看link:../snapmirror-active-sync/mediator-install-task.html["为 SnapMirror 主动同步配置 ONTAP 调解器和集群"]。
