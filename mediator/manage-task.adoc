...
[CAUTION]
====
* 只有某些ONTAP版本上的ONTAP调解器才支持第三方证书。请参阅。 link:https://mysupport.netapp.com/site/bugs-online/product/ONTAP/JiraNgage/CONTAP-243278["NetApp错误在线中的错误ID COBTP-243278"^]
...
NOTE: 对于SnapMirror业务连续性(SM-BC)集群、必须将和证书添加 `intermediate.crt` `root_ca.crt` 到ONTAP集群。请参阅。 link:../snapmirror-active-sync/mediator-install-task.html["为ONTAP活动同步配置SnapMirror调解器和集群"]
