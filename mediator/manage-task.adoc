...
如果运行ONTAP Mediator 的 VM 需要使用较新的内核进行升级，则这可能会导致与ONTAP Mediator 使用的 SCST 内核模块出现兼容性问题。在这种情况下， NetApp建议您手动卸载并重新安装 SCST。
...
NOTE: 如果您是首次安装并且想要在安装ONTAP Mediator 之前预安装 SCST，则可以跳过此步骤。  ONTAP调解器安装程序在安装过程中将任何相关修补程序应用于 SCST 组件。
