...
从 ONTAP 9.10.1 开始，您可以启用 System Manager 的 " 自动更新 " 功能，以使 ONTAP 能够自动下载和安装 NetApp 提供的建议软件修补程序，升级和更新（默认行为）。

更新包括以下类别的固件修补程序，升级和更新：

* * 存储固件 * ：存储设备，磁盘和磁盘架
* * SP/BMC 固件 * ：服务处理器和 SP/BMC 模块


在 System Manager 中，您可以更改每个类别的默认行为，以便从 Active IQ 数字顾问处收到固件修复和更新建议，从而可以确定要安装的固件，并在需要安装时设置计划。您也可以关闭此功能。

要计划自动执行修复和更新以及接收有关更新的建议，请执行以下工作流任务：

image:../media/sm-firmware-auto-update.gif["自动更新工作流"]

要在 System Manager 中接收有关固件更新的警报，您必须向 Active IQ 数字顾问注册。请参见 https://docs.netapp.com/us-en/active-iq/index.html["Active IQ 数字顾问文档"]。
...
要启用 System Manager 的自动更新功能，您必须接受 NetApp 指定的条款和条件。
...
在 System Manager 中，您可以查看 Active IQ 提供的建议列表，并一次性对每个建议或所有建议执行操作。
