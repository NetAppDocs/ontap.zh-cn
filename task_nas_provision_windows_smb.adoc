...
keywords: netapp, ontap, provisioning, nas, windows, servers, provision, smb, cifs, SMB, share name,shares, share access permissions, share access properties, hyper-v over smb, sql server over smb, continuously available shares, system manager 
summary: 使用 ONTAP 系统管理器创建卷，以便使用 SMB 协议为 Windows 服务器提供存储，该系统管理器可在 ONTAP 9.7 及更高版本的 ONTAP 9 中使用。 
---
...
[role="lead"]
使用 ONTAP 系统管理器创建卷，以便使用 SMB 协议为 Windows 服务器提供存储，该系统管理器可在 ONTAP 9.7 及更高版本的 ONTAP 9 中使用。
