...
summary: 创建卷以使用 SMB/CIFS 协议为 Windows 服务器提供存储。 
---
= 使用 SMB/CIFS 为 Windows 服务器配置 NAS 存储
:toc: macro
:hardbreaks:
:toclevels: 1
:toc: 
...
[role="lead"]
创建卷以使用 SMB/CIFS 协议为 Windows 服务器提供存储。

此操作步骤会在上创建新卷 link:task_nas_enable_windows_smb.html["已启用 SMB 的现有 Storage VM"]。您可以在配置卷或指定自定义配置时接受系统默认值。
...
验证访问后，您可以 link:task_nas_provision_export_policies.html["使用共享 ACL 限制客户端访问"] 并在映射的驱动器上设置所需的任何安全属性。
...
NOTE: 保存卷后，返回到 <<step2>> 使用 SMB/CIFS 完成 Windows 服务器的配置。
