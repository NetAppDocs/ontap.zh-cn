...
从ONTAP 9.15.1开始、在自动配置FlexGroup卷时、ONTAP会使用平衡放置配置来确保空间保证有足够的可用空间。使用REST API或创建FlexGroup卷时 `auto-provision-as` 使用ONTAP命令行界面时、由于强制实施空间保证而导致空间不足、配置可能会失败。您可以通过创建较小的FlexGroup卷或来避免这种情况 link:create-task.html["创建FlexGroup卷并手动选择聚合"] 使用 `aggr-list` 参数。
