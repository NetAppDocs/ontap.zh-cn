...
* 在启用高级容量平衡之前，请禁用 SMB Multichannel。将 SMB Multichannel 与高级容量重新平衡结合使用可能会导致高延迟。有关详细信息，请参见 link:https://kb.netapp.com/on-prem/ontap/da/NAS/NAS-Issues/CONTAP-400433["CONTAP-400433：在启用 SMB 多通道的客户端上使用 FlexGroup Rebalancing/GDD 时，读/写延迟较高"^]。
