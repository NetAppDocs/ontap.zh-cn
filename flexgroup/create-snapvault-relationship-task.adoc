...
.. 为SnapVault策略定义一条规则、用于确定在初始化和更新操作期间传输哪些Snapshot副本： `snapmirror policy add-rule -vserver svm_name -policy policy_for_rule - snapmirror-label snapmirror-label -keep retention_count -schedule schedule`
