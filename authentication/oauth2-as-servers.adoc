...
.如何定义授权服务器
您可以使用任何管理界面（包括 ONTAP CLI、System Manager 或 ONTAP REST API）为 ONTAP 定义授权服务器。例如，使用 ONTAP CLI 时，您可以使用命令 `security oauth2 client create`。
...
授权服务器颁发的 OAuth 2.0 访问令牌由 ONTAP 验证，并用于为 ONTAP REST API 客户端请求做出基于角色的访问决策。



=== 获取访问令牌

您需要从为使用 ONTAP REST API 的 ONTAP 集群定义的授权服务器获取访问令牌。要获取令牌，您必须直接联系授权服务器。
