...
summary: 从 ONTAP 9.14 开始，您可以选择使用开放授权（OAuth 2.0）框架控制对 ONTAP 集群的访问。您可以使用任何 ONTAP 管理界面配置此功能，包括 ONTAP CLI、System Manager 和 ONTAP REST API。但是，OAuth 2.0 授权和访问控制决策只能在客户端使用 ONTAP REST API 访问 ONTAP 时应用。 
---
= ONTAP OAuth2.0实施概述
:hardbreaks:
:allow-uri-read: 
...
[role="lead"]
从 ONTAP 9.14 开始，您可以选择使用开放授权（OAuth 2.0）框架控制对 ONTAP 集群的访问。您可以使用任何 ONTAP 管理界面配置此功能，包括 ONTAP CLI、System Manager 和 ONTAP REST API。但是，OAuth 2.0 授权和访问控制决策只能在客户端使用 ONTAP REST API 访问 ONTAP 时应用。
