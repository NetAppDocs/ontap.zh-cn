...
summary: 从 ONTAP 9.16.1 开始，您可以启用和禁用 NVMe TCP 连接的 TLS 安全通道。 
---
= 为NVMe/TCP设置TLS安全通道
:allow-uri-read: 
:icons: font
...
[role="lead"]
从 ONTAP 9.16.1 开始，您可以为 NVMe/TCP 连接配置 TLS 安全通道。您可以使用 System Manager 或 ONTAP CLI 添加启用了 TLS 的新 NVMe 子系统，或为现有 NVMe 子系统启用 TLS。从 ONTAP 9.19.1 开始，支持 TLS 硬件卸载。
