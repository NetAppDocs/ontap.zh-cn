...
. (可选)在Storage VM对等方下、单击、然后单击 image:icon_kabob.gif["菜单选项图标"] *对等Storage VM*以对等Storage VM。
. 单击 * 保护卷 * 以保护卷。要保护LUN，请单击*存储> LUN*，选择要保护的LUN，然后单击 image:icon_protect.gif["保护图标"]。
+
根据需要的数据保护类型选择保护策略。
