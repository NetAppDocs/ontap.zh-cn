...
ONTAP 支持用于远程用户身份验证的安全断言标记语言 (SAML)。支持多个常见身份提供程序 (IdPs)。有关支持的 IdPs 和启用 SAML 身份验证的说明的更多信息，请参阅 link:../system-admin/configure-saml-authentication-task.html["配置 SAML 身份验证"]。



== OAuth2.0与ONTAP REST API客户端

从 ONTAP 9.14 开始，支持开放式授权（OAuth 2.0）框架。仅当客户端使用 ONTAP REST API 访问 ONTAP 时，才能使用 OAuth 2.0 进行授权和控制访问决策。但是，您可以使用任何 ONTAP 管理界面配置和启用该功能，包括 ONTAP CLI、System Manager 和 ONTAP REST API。
