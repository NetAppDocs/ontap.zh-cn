...
NOTE: 从 ONTAP 9.15.1 开始，您可以使用 SCSI-3 为虚拟卷创建永久预留。此功能仅支持将 ONTAP tools for VMware vSphere 9 与 Windows Server 故障转移群集 (WSFC) 配合使用。
...
VMware Live Site Recovery 以前称为 Site Recovery Manager (SRM)，提供灾难恢复功能。Live Site Recovery 与 ONTAP tools for VMware vSphere 集成，以访问和利用 ONTAP 数据管理功能。
