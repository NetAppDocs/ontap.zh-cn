...
summary: 许多 NetApp 客户存储了大量很少被访问的数据。我们称之为冷数据。客户还拥有频繁访问的数据，我们称之为热数据。理想情况下，您希望将热数据保存在最快的存储上，以获得最佳性能。冷数据可以移动到较慢的存储，只要它在需要时立即可用。了解如何知道您数据的哪些部分是热的，哪些部分是冷的。 
---
= FabricPool
:allow-uri-read: 
:icons: font
...
.相关信息
link:../concept_cloud_overview.html["FabricPool 系统管理器文档"]
