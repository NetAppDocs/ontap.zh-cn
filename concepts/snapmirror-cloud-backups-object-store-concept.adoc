...
[role="lead"]
_SnapMirror cloud_ 是一种备份和恢复技术，专为希望将其数据保护工作流转换到云或本地对象存储解决方案的 ONTAP 用户而设计。正在摆脱传统磁带备份架构的组织可以使用对象存储作为长期数据保留和存档的替代存储库。SnapMirror cloud 提供 ONTAP 到对象存储的复制，作为增量永久备份策略的一部分。
