...
对于以下平台，默认情况下，存储效率仅在升级到 ONTAP 9.15.1 或更高版本之前已启用存储效率的现有精简配置卷上启用。您可以使用 ONTAP CLI 或 ONTAP REST API 方法在新创建的卷上启用存储效率。只有在原始平台上启用了压缩的情况下，使用卷移动或 SnapMirror 技术迁移的数据才会转换为 32k 内联压缩。
