...
+
这 `-version` 选项指的是您要恢复到的ONTAP版本。只需要ONTAP版本的前两个值。例如，如果您要从 9.14.1 版本回滚到 9.13.1 版本，则正确的值为： `-version` 选项为 9.13。
...
NOTE: 重新启用集群 HA 可能导致 `storage failover auto-giveback`设置恢复为 true。
...
----
. 验证两个节点上的 `storage failover auto-giveback`设置：
+
[source, cli]
----
...
----
+
如果您的环境需要不同的设置，请使用 `storage failover modify -node <nodename> -auto-giveback <true|false>`命令进行修改。
...
* link:https://kb.netapp.com/on-prem/ontap/Ontap_OS/OS-KBs/Why_auto-giveback_enabled_after_re-enabling_storage_failover_setting["NetApp 知识库：为什么在重新启用存储故障转移设置后启用自动交还"^]
