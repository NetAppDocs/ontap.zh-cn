...
* link:arp-warning-clear-revert.html["对异常活动的自动防系统异常警告做出响应"].




| ONTAP 9.19.1  a| 
* 如果您有任何具有主动-主动多路径配置的 SVM，则必须 link:delete-active-active-svms.html["删除 SVM"]。
* 如果您修改了在 ONTAP 9.19.1 或更高版本中创建的任何 QoS 策略组的默认设置，则必须 link:modify-qos-policy-groups.html["重置它们"]。
* 如果您将卷升级到 ONTAP 9.19.1，并向任何用户添加了第二个 S3 访问密钥，则必须 link:second-s3-access-key.html["删除这些密钥"]。




| ONTAP 9.18.1  a| 
* 如果在 ONTAP 9.18.1 升级过程中已为 ARP/AI 设置了自动启用，则需要 link:anti-ransomware-disable-auto-enable.html["将其禁用"]。




| ONTAP 9.17.1  a| 
* 如果您已为 SAN 启用 ONTAP ARP/AI 功能，link:anti-ransomware-disable-san.html["将其禁用"]。
