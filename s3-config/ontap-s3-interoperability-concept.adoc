...
* 嵌套 QoS 策略
+
link:../performance-admin/policy-group-assignment-restrictions.html["QoS 策略组分配限制"]
...
其他功能
 a| 
* 支持对构成 FlexGroup 卷的单个成员卷执行卷移动操作。
...
* FabricPool 云层 (link:../s3-config/index.html["仅限本机 S3"])
* FabricPool 本地层 (link:../s3-multiprotocol/index.html["仅 NAS 卷"])
* link:../flexcache/supported-unsupported-features-concept.html#supported-features["FlexCache 卷"] （从 ONTAP 9.18.1 开始）

 a| 
* S3 存储区使用的 FlexGroup 卷不支持卷移动。
* fpolicy
* qtree
* 配额
