...
为了在原始访问密钥过期时维护对 S3 资源的访问，从 ONTAP 9.19.1 开始，您可以为每个 S3 用户link:regenerate-access-keys.html#generate-a-second-s3-access-key["生成第二个访问密钥"]。
...
----
+
在  `-access-key` 和  `-secret-key` 参数中创建用户时，您也可以手动定义自己的访问密钥和密钥。例如：
...
.System Manager
--
. 选择 *Storage > Storage VMs*。选择需要添加用户的 storage VM，选择 *Settings*，然后在 S3 下选择 image:icon_pencil.gif["编辑图标"]。
. 要添加用户，请选择 *Users > Add*。
. 输入用户的名称。
