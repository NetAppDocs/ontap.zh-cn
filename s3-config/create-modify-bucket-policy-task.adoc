...
从 ONTAP 9.19.1 开始，您可以使用条件强制对象上传的条件写入和对象删除的条件删除。条件写入和删除提供了一种防止覆盖或意外删除对象的方法。
...
----
从 ONTAP 9.19.1 开始，您可以使用条件强制对象上传的条件写入和对象删除的条件删除。以下示例创建存储区策略语句条件以强制执行 bucket1 的条件写入：
