...
| PutBucketLifecycleConfiguration | ONTAP 9.13.1 * 仅支持过期操作 
...
从 ONTAP 9.19.1 开始，ONTAP S3 支持强制执行对象上传的条件写入和对象删除的条件删除。条件写入和删除提供了一种防止覆盖或意外删除对象的方法。只有在通过 TLS 连接使用 AWS_V4 签名方法发出请求时，条件操作才有效。支持以下条件标头和对象操作：
...
| DeleteObjects ONTAP S3 支持此附加标头：* `x-amz-bypass-governance-retention` | ONTAP 9.11.1 
...
* ONTAP S3 增加了对分块上传签名请求的支持，使用 `x-amz-content-sha256:
