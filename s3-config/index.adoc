...
[role="lead"]
从 ONTAP 9.8 开始，您可以在 ONTAP 集群中启用 ONTAP 简单存储服务（ S3 ）对象存储服务器。有关详细信息，请参见 xref:s3-support-concept.html[ONTAP 9 中的 S3 支持]。
...
有关 FabricPool 分层的详细信息，请参见 xref:../concept_cloud_overview.html[使用 System Manager 进行 FabricPool 层管理概述]。
