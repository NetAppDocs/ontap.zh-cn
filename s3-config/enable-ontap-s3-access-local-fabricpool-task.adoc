...
. 为本地容量层创建对象存储：
+
`storage aggregate object-store config create -object-store-name store_name -ipspace cluster -provider-type ontap_s3 -server s3_server_name -container-name bucket_name -access-key access_key -secret-password password password`
