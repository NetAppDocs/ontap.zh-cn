...
*** 如果未指定新存储桶的聚合和组成部分，则新存储桶将被添加到现有 FlexGroup，但仅当 `retention-mode`设置与现有存储桶的 `retention-mode`设置匹配且 FlexGroup 类型为(`compliance`、 `non-snaplock`、 `enterprise`）时。有关详细信息，请参见xref:../flexgroup/index.html[FlexGroup 卷管理]。
+
