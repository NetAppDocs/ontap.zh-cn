...
| 更新 | Description 
 ...
 a| 
xref:../snapmirror-active-sync/[SnapMirror主动同步的节点支持]
 a| 
...
多协议S3存储分段支持S3对象元数据和标记
 a| 
S3对象标记可用于非多协议ONTAP S3存储分段。从ONTAP 9.161开始、S3对象标记功能已扩展到NAS和S3多协议ONTAP S3存储分段。这些标记仅在S3协议中可见。使用S3客户端应用 https://docs.aws.amazon.com/AmazonS3/latest/userguide/object-tagging.html["标记和元数据"^]于S3对象有助于定义生命周期、回付费用、数据类别以及对ONTAP中作为对象或文件存储的数据的自定义工作流。与Breck或Athena等AWS数据服务集成后、标记和元数据将成为这些服务提供的Data Processing的核心。
