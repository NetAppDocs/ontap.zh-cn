...
| 自主勒索软件防护（ARP/AI） | 在受支持的平台上，经过 12 小时的宽限期后，默认在所有新卷上自动启用 ARP/AI。 | ONTAP 9.18.1 
...
| ONTAP SVM 缓存正凭据的生存时间 (TTL) | 从 ONTAP 9.17.1 开始，ONTAP SVM 缓存的正向凭据的生存时间（TTL）已从 24 小时（86,400,000 毫秒）减少为 1 小时（3,600,000 毫秒）。 | ONTAP 9.17.1 
...
| SnapMirror云 | SnapMirror cloud 将支持的 S3 存储桶数量从 6 个增加到 100 个。 | ONTAP 9.19.1 


| S3 连接流量控制限值 | 对于内存 <= 32GB 的云实例，ONTAP S3 连接的流量控制限制已从 2 MB 降至 256 KB。 | ONTAP 9.19.1 


| S3 空闲连接超时 | S3 空闲连接超时已从 20 分钟缩短到 5 分钟。 | ONTAP 9.19.1 
...
| 集群对等 | 支持的集群对等限制从 255 个增加到 400 个，提高了具有大量集群对等关系的环境的可扩展性。 | ONTAP 9.14.1 
...
* FlexGroup 中的成分数量上限不能超过 24 个。
* SVM-DR 卷限制为每个节点 500 个，每个 HA 对 1000 个（包括 FlexVol 卷和 FlexGroup 成分卷）。
+

NOTE: 限制因平台而异。您应通过转到 link:https://hwu.netapp.com["NetApp Hardware Universe"^] 并选择您的平台和 ONTAP 版本来验证适合您配置的正确限制。
...
| SnapMirror并发传输 | 异步卷级并发传输的最大数量已从 100 增加到 200。高端系统上的SnapMirror云传输次数从 32 次增加到 100 次，低端系统上的SnapMirror传输次数从 6 次增加到 20 次。 | ONTAP 9.8 
