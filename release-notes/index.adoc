...
* 了解 https://docs.netapp.com/us-en/ontap-afx/release-notes/whats-new-9191.html["适用于NetApp AFX 存储系统的全新增强型ONTAP软件功能"^]。
* 了解 https://docs.netapp.com/us-en/asa-r2/release-notes/whats-new-9191.html["适用于NetApp ASA r2 存储系统的全新增强型ONTAP软件功能"^]。
...
ONTAP 9.19.1 在安全性、可扩展性和存储管理领域提供了新增和增强功能，以支持数据基础设施的现代化。这些增强功能可帮助您扩展规模、提高运营效率，并确保未来数据的安全。有关新功能和增强功能的完整列表，请参阅 xref:whats-new-9191.adoc[ONTAP 9.19.1 中的新增功能]。

* xref:../performance-admin/guarantee-throughput-qos-task.html[可配置的QoS突发策略]
+
可配置的 QoS 突发策略允许受控的、有时间限制的性能突发超过 QoS 限制，在不牺牲公平性或要求过度配置的情况下，提高峰值事件期间的工作负载响应速度。

* xref:../nfs-admin/enable-nfs-over-tls-task.html[基于 TLS 1.3 的 NFS]
+
使用基于标准的 TLS 1.3 加密保护传输中的 NFS 数据，有助于在企业规模上加强保护并简化加密 NFS 的采用。 

* xref:../s3-config/regenerate-access-keys.html[每个用户有多个 S3 访问密钥]
+
增加了对每个用户多个活动 S3 访问密钥的支持，实现无缝凭证轮换而无需应用程序停机，帮助客户在保持对对象工作负载不间断访问的同时加强安全性和合规性。

* xref:../snapmirror-active-sync/protect-nas-task.html[SnapMirror 适用于 NAS 的活动同步]
+
借助 NAS 支持，SnapMirror 主动同步现在是一个统一的业务连续性解决方案，通过透明故障转移提供零 RPO。 

* xref:../san-config/configure-fc-nvme-hosts-ha-pairs-reference.html[FC直连]
+
引入安全 FC HBA 卡支持，实现可信主机连接，并为硬件支持的传输中 FC 安全性奠定基础，同时保持性能并支持企业合规性要求。 

* xref:../san-admin/multipath-automatic-lif-failover-support.html[统一主动-主动 LIF 故障转移]
+
AFF 系统通过自动 LIF 故障转移提供主动-主动多路径，使所有 SAN 路径保持活动优化状态，从而确保主机 I/O 在故障转移期间立即继续运行。 

* link:https://docs.netapp.com/us-en/ontap-fli/get-started/learn-about-data-migration.html["FLI 中的 iSCSI 支持"^]
+
添加了对外部 LUN 导入 (FLI) 的 iSCSI 支持，支持使用现有的 iSCSI 连接将块数据从第三方存储系统迁移到 ONTAP，同时将中断降至最低。 

* xref:../s3-config/create-modify-bucket-policy-task.html[S3 条件写入和删除]
+
使用 S3 API 启用对条件写入和删除的支持，允许应用程序在对象更新和删除期间强制执行数据一致性检查，从而降低并发和分布式对象工作负载中意外覆盖或删除的风险。

* xref:../concepts/path-failover-concept.html#nas-path-failover[组合 LIF 故障转移]
+
AFF 和 ASA 系统支持自动 LIF 故障转移，适用于主动-主动和主动-主动-仅 scsi 多路径配置中的 iSCSI 协议 LIF，以及主动-主动多路径配置中的 NVMe/TCP 协议 LIF。 

* xref:../networking/improve-tcp-wan-performance.html[增强的 WAN 性能]
+
使用 TCP RACK 增强的 WAN 性能提高了拥塞网络上的吞吐量和弹性，加快了远程文件访问和站点间数据服务。 

* xref:../networking/configure-tls-offload.html[硬件加速 kTLS，实现更快的后端集群加密]
+
通过引入硬件加速 kTLS 增强后端集群加密，将加密处理卸载到受支持的 SmartNIC，以提供更高的性能和更低的 CPU 开销，同时保持集群和 HA 流量之间始终开启的安全通信。 

* xref:../anti-ransomware/use-cases-restrictions-concept.html#snapmirror[针对同步复制工作负载的 AI 驱动勒索软件检测]
+
将自主勒索软件保护（ARP/AI）扩展到适用于（SAN 和 NAS）的 SnapMirror 同步以及 SnapMirror 活动同步 SAN 环境，在零 RPO 架构中保留检测和恢复信号。 

* xref:../concept-svm-admin-dashboard-sm.html[使用新的 ONTAP System Manager 仪表板监控并管理 SVM]
+
为 SVM 管理提供对租户友好的 GUI，无需复杂的 CLI 界面。 
...
ONTAP 9.18.1 在安全性、可扩展性和存储管理方面提供了新的和增强的功能，以支持数据基础设施现代化。这些增强功能可帮助您扩展规模、提高运营效率并确保未来数据安全。如需查看所有新增功能和改进的完整列表，请参阅xref:whats-new-9181.adoc[ONTAP 9.18.1 的新增功能]。

* xref:../networking/configure-backend-encryption.html[后端集群高可用性加密]
+
能够使用 mTLS 对后端集群网络进行加密。

* xref:../networking/configure_network_security_using_federal_information_processing_standards_fips.html[PQC对传输中数据的支持]
+
能够使用后量子算法处理传输中的数据。

* xref:../networking/ipsec-prepare.html[支持 IPv6 IPSec 和硬件卸载]
+
支持使用 IPv6 地址进行硬件卸载的 IPSec。

* xref:../anti-ransomware/use-cases-restrictions-concept.html#supported-configurations[ARP/AI 支持FlexGroup卷]
+
NetApp FlexGroup卷支持 ARP/AI，提供基于机器学习的高级勒索软件检测和保护。

* xref:../san-admin/nvme-copy-offload.html[NVMe 的复制卸载]
+
通过将复制操作直接卸载到存储设备来加速大数据传输，从而降低主机 CPU 和网络负载。

* xref:../s3-snapshots/list-restore-s3-snapshot.html[存储驱动的 S3 存储桶快照恢复]
+
...
+
ARP/AI 通过基于加密的异常检测支持 SAN 卷，引入用于详细熵统计的新命令，并在 System Manager 中统一勒索软件保护消息传递（此前该功能主要面向 NAS）。可配置的检测阈值和更具确定性的快照保留为不同的工作负载提供了更大的灵活性。
...
+
ONTAP 9.16.1 向 MAV 框架添加了更多命令，以进一步防范恶意内部人员。这些增强功能包括许多一致性组 (CG)、VScan 和自治勒索软件保护 (ARP/AI) 管理以及 NVMe 配置命令。

* xref:../anti-ransomware/index.html[利用AI增强功能(ARP/AI)提供自主防兰森功能]
+
自主勒索软件防护已升级，具备全新 AI 功能，能够以 99% 的精确率和召回率检测和响应勒索软件攻击。由于 AI 是在全面的数据集上训练的，因此在 FlexVol 卷上运行的 ARP/AI 不再需要学习期，ARP/AI 会立即以活动模式启动。ARP/AI 还引入了独立于 ONTAP 升级的自动更新功能，以确保针对最新威胁的持续保护和弹性。
...
+
从 ONTAP 9.15.1 开始，您还可以选择备份符号链接本身，而不是它指向的数据。这可以提供多种好处，包括提高备份应用程序的性能。您可以使用 ONTAP CLI 或 ONTAP REST API 启用此功能。
...
** xref:../anti-ransomware/support-arpai.html#multi-admin-verification-with-arp-ai-protected-volumes[支持多管理员验证]
+
...
+
如果 FlexGroup 成分变得不平衡，可以从 ONTAP CLI、ONTAP REST API 和 System Manager 对 FlexGroup 进行无中断重新平衡和管理。为了获得最佳性能，FlexGroup 中的成员组件应均匀分布其已用容量。
