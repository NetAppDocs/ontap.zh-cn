...
xref:../smb-admin/windows-backup-symlinks.html[服务器上的 Windows 备份应用程序和 Unix 风格的符号链接]
 a| 
当 Windows 备份应用程序遇到 Unix 风格的符号链接（symlink）时，将跟随该链接，实际数据由 ONTAP 返回并进行备份。从 ONTAP 9.15.1 开始，您还可以选择备份符号链接本身，而不是它所指向的数据。这可以提供多种好处，包括提高备份应用程序的性能。您可以使用 ONTAP CLI 或 ONTAP REST API 启用此功能。
...
现在、使用SnapMirror异步保护的一致性组最多可支持一致性组中的80个卷。
 ...
 a| 
xref:../consistency-groups/configure-task.html[支持具有一致性组的 ONTAP REST API 和 ONTAP CLI 操作的管理员权限级别]
 a| 
现在，管理权限级别支持一致性组的 ONTAP CLI 和 ONTAP REST API 操作。
...
xref:../concepts/ontap-and-vmware.html[使用Windows Server故障转移集群为VMware虚拟卷保留永久性预留空间]
 a| 
ONTAP 当前支持 VMware 虚拟卷（vVols）以及对传统 LUN 的永久预留。从 ONTAP 9.15.1 开始，您还可以使用 vVol 创建永久预留。对此功能的支持在 ONTAP tools for VMware vSphere 9 中实现。它仅在 Windows Server 故障转移群集（WSFC）中受支持，该群集是一组群集化的 Windows 虚拟机。
