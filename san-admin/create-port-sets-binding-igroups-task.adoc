...
summary: 除了使用 Selective LUN Map 之外，您还可以创建端口集并将该端口集绑定到 igroup，以进一步限制启动器可以使用哪些 LIF 来访问 LUN。如果不将端口集绑定到 igroup，则 igroup 中的所有启动器都可以通过拥有 LUN 的节点和拥有节点的 HA 伙伴上的所有 LIF 来访问映射的 LUN。 
---
= 创建端口集并绑定到 igroup
:allow-uri-read: 
:icons: font
