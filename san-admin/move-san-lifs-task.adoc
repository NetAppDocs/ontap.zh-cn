...
summary: 如果节点需要脱机，则可以移动 SAN LIF 以保留其配置信息（例如其 WWPN），并避免重新分区交换机结构。由于 SAN LIF 必须在移动之前脱机，因此主机流量必须依靠主机多路径软件来提供对 LUN 的无中断访问。您可以将 SAN LIF 移动到群集中的任何节点，但不能在存储虚拟机之间移动 SAN LIF。 
---
= 移动 SAN LIF
:allow-uri-read: 
:icons: font
