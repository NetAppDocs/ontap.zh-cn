...
summary: 从 ONTAP 9.4 开始，SAN 环境中支持 NVMe FC。NVMe FC 使存储管理员能够调配命名空间和子系统，然后将命名空间映射到子系统，类似于为 FC 和 iSCSI 调配 LUN 并将其映射到 igroup 的方式。 
---
= 使用 NVMe 进行 SAN 配置
:allow-uri-read: 
:icons: font
...
.相关信息
https://www.netapp.com/pdf.html?item=/media/10681-tr4684.pdf["NetApp 技术报告 4684 ：《使用 NVMe/FC 实施和配置现代 SAN 》"^]
