...
summary: Microsoft Offloaded Data Transfer，也称为 copy offload，可在存储设备内或兼容存储设备之间实现直接数据传输，而无需通过主机计算机传输数据。 
---
...
在虚拟化环境中使用 VMware 和 Microsoft 复制卸载时，您的 LUN 必须对齐。未对齐的 LUN 会降低性能。link:io-misalignments-properly-aligned-luns-concept.html["了解更多关于未对齐 LUN 的信息"]。
