...
summary: '从纯闪存SAN阵列(ASA)平台上的ONTAP 9.11.1开始、iSCSI LIF故障转移功能支持在SFo配对节点故障转移(iSCSI LIF从主节点/端口移至HA配对节点/端口并重新移回)和本地故障转移中自动和手动迁移iSCSI LIF (iSCSI LIF从运行状况不正常的端口移至当前主节点上运行状况正常的端口并重新移回时)。此功能可加快iSCSI上运行的SAN工作负载的I/O恢复速度。' 
---
...
[role="lead"]
从纯闪存SAN阵列(ASA)平台上的ONTAP 9.11.1开始、iSCSI LIF故障转移功能支持在SFo配对节点故障转移(iSCSI LIF从主节点/端口移至HA配对节点/端口并重新移回)和本地故障转移中自动和手动迁移iSCSI LIF (iSCSI LIF从运行状况不正常的端口移至当前主节点上运行状况正常的端口并重新移回时)。此功能可加快iSCSI上运行的SAN工作负载的I/O恢复速度。
...
* iSCSI LIF故障转移和迁移在LIF主端口的广播域内进行。使用查看广播域配置 `network port broadcast-domain show` 命令：
+

IMPORTANT: 确保与iSCSI LUN关联的广播域包含表示HA对中两个节点的网络端口、以便可以根据需要在两个节点之间迁移和故障转移iSCSI LUN。
...
.关于此任务
请参见 https://docs.netapp.com/us-en/ontap-cli-9141/index.html["《ONTAP 命令参考》"^] 查看的完整列表 `network interface` 命令
