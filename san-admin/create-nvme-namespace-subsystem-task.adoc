...
[role="lead"]
调配存储时，可以使 NVMe 主机能够从 ONTAP 存储系统读取和写入数据。对于使用 NVMe 协议的系统，必须创建一个或多个 NVMe 命名空间和子系统。然后，可以将每个命名空间映射到 NVMe 子系统，以允许从主机访问数据。

如果包含 NVMe 子系统的存储虚拟机 (SVM) 同时具有 NVMe/FC 和 NVMe/TCP 网络接口，则映射到 NVMe 子系统的命名空间可以通过 NVMe/FC 和 NVMe/TCP 传输协议进行访问。从 ONTAP 9.4 及更高版本开始提供 NVMe 对 FC 的支持。ONTAP 9.10.1 及更高版本提供 NVMe 对 TCP 的支持。如果您需要对命名空间进行特定于协议的访问，则应为每个协议link:configure-svm-nvme-task.html["配置 SVM"]。
