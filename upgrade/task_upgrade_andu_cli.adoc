...
NOTE: 如果要在MetroCluster FC配置中升级ONTAP、则还需要验证集群是否已启用自动计划外切换。
...
* 您应启动 Active IQ 数字顾问。
+
Active IQ Digital Advisor 的 Upgrade Advisor 组件可帮助您规划成功的升级。

+
Active IQ Digital Advisor 会向所有与 *SupportEdge* 签有有效合同的 NetApp 客户提供数据驱动型洞察和建议（功能因产品和支持层而异）。
...
.关于此任务
。 `cluster image validate` 命令会检查集群组件以验证升级是否可以无中断完成、然后提供每个检查的状态以及执行软件升级之前必须执行的任何必要操作。
...
----
+
如果您要执行 link:https://docs.netapp.com/us-en/ontap/upgrade/concept_upgrade_paths.html#types-of-upgrade-paths["直接多跳升级"]，您还需要下载升级所需的ONTAP中间版本的软件包。例如、如果要从9.8升级到9.13.1、则需要下载适用于ONTAP 9.12.1的软件包、然后使用同一命令下载适用于9.13.1.的软件包。
...
.关于此任务
如果要手动完成升级、请使用 `cluster image cancel-update` 命令以取消自动过程并手动继续。如果要继续自动升级，请完成以下步骤。
