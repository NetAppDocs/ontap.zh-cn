...
.2+| 9.16.1 | 9.18.1 | supported 


| 9.17.1 | supported 


.3+| 9.15.1. | 9.18.1 | supported 
...
.步骤
. link:concept_upgrade_methods.html["升级"] 不支持新控制器所需的最低ONTAP版本的节点，应升级到它们支持的最高ONTAP版本。
+
例如，如果您有一台运行ONTAP 9.5 的 FAS8080，并且您正在添加一台运行ONTAP 9.12.1 的新 C 系列平台，则应将 FAS8080 升级到ONTAP 9.8（这是它支持的最高ONTAP版本）。
...
. link:../system-admin/remove-nodes-cluster-concept.html["从集群中删除不受支持的节点"]。
. link:concept_upgrade_methods.html["升级"] 将集群迁移到与新节点上运行的ONTAP版本和补丁级别相同的环境，或者迁移到…… link:https://kb.netapp.com/Support_Bulletins/Customer_Bulletins/SU2["最新建议的修补程序版本"^] 适用于在新节点上运行的ONTAP版本。
. 请确认所有节点运行的ONTAP版本是否相同。
+
.. 显示集群上运行的ONTAP版本：
+
[source, cli]
...
+
如果输出中报告的ONTAP版本存在差异，则 `version *` （集群）和 `version` （单个节点）命令，通过运行以下命令将所有节点更新到相同的ONTAP和补丁版本： link:concept_upgrade_methods.html["集群镜像更新"]。
