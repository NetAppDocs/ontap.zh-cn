...
[role="lead"]
执行自动无中断升级 (ANDU) 时，ONTAP 自动在每个节点上安装目标 ONTAP 映像，验证集群是否可以成功升级，然后根据集群中的节点数量在后台执行 xref:concept_upgrade_methods.html[批量升级或滚动升级]。
...
IMPORTANT: 在开始 ANDU 之前修改 `storage failover modify-auto-giveback`命令选项的设置对升级过程没有影响。在更新所需的接管/回馈期间，ANDU 过程将忽略此选项的任何预设值。例如，在开始 ANDU 之前将 `-autogiveback`设置为 false，不会在回馈之前中断自动升级。如需了解有关 `storage failover modify-auto-giveback`的更多信息，请参阅link:https://docs.netapp.com/us-en/ontap-cli/search.html?q=storage+failover+modify-auto-giveback["ONTAP 命令参考"^]。
...
* 或者，link:../update/pre-stage-system-firmware.html["为单次重启升级预暂存系统固件"]。
+
从 ONTAP 9.17.1 开始，如果在执行 ANDU 之前，节点的启动设备上已存在较新的 BIOS 或 LOADER 固件，则升级过程将保留较新的固件，而不是用 ONTAP 映像中捆绑的版本覆盖它。这样，ONTAP 软件升级和固件更新均可在一个重新启动周期内完成。
...
+
对于匿名 FTP，您必须以 \ftp://anonymous@ftpserver 格式指定 URL。
...
.. 完成验证确定的所有必需操作。
+
系统将显示完整的自动升级预检查列表，以及在开始升级过程之前应解决的任何错误或警告。在继续升级之前，您必须解决所有错误。最佳做法是同时解决警告。
...
====
.下一步是什么？
如果您预先暂存了固件，请使用 `system node firmware bootmedia show`命令验证每个节点上的预期固件版本是否处于活动状态。
