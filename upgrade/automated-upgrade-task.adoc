...
* 如果您不打算监控升级过程的进度、则应这样做 link:task_requesting_notification_of_issues_encountered_in_nondisruptive_upgrades.html["请求可能需要手动干预的错误的 EMS 通知"]。
...
. 如果要升级MetroCluster FC配置、请验证集群是否已启用自动计划外切换。
+

NOTE: 如果要升级标准配置或MetroCluster IP配置、则无需执行此步骤。
