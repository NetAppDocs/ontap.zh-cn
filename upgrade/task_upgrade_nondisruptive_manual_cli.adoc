...
---
= 使用命令行界面手动执行无中断ONTAP升级(非MetroCluster系统)
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
. 通过调用AutoSupport消息更新集群中的第一个节点： `autosupport invoke -node * -type all -message "Starting_NDU"`
+
...
. 将新的ONTAP软件映像设置为默认映像： `system image modify {-node nodenameA -iscurrent false} -isdefault true`
+
system image modify 命令使用扩展查询将新的 ONTAP 软件映像（作为备用映像安装）更改为节点的默认映像。

. 监控更新进度： `system node upgrade-revert show`
...
----
. 在配对节点上禁用自动交还(如果已启用)： `storage failover modify -node nodenameB -auto-giveback false`
+
...
----
. 运行以下命令两次、以确定要更新的节点当前是否正在为任何客户端提供服务 `system node run -node nodenameA -command uptime`
+
...
. 验证已迁移的任何SIFs： `network interface show`
+
...
. 验证是否已归还所有聚合： `storage failover show-giveback`
+
...
.. 验证节点的更新状态是否为complete： `system node upgrade-revert show -node nodenameA`
+
此状态应列为已完成。
...
----
. 将这些生命周期恢复到节点： `network interface revert *`
+
此命令将返回已从节点迁移的 LIF 。
...
----
. 验证节点的数据生命周期是否已成功还原回节点、并且它们是否已启动： `network interface show`
+
...
4 entries were displayed.
----
. 如果您先前已确定此节点为客户端提供服务、请验证此节点是否正在为其先前提供的每个协议提供服务： `system node run -node nodenameA -command uptime`
+
更新期间，操作计数重置为零。
...
. 将新的ONTAP软件映像设置为默认映像： `system image modify {-node nodenameB -iscurrent false} -isdefault true`
+
system image modify 命令使用扩展查询将新的 ONTAP 软件映像（作为备用映像安装）更改为节点的默认映像。
...
4 entries were displayed.
----
. 在配对节点上禁用自动交还(如果已启用)： `storage failover modify -node nodenameA -auto-giveback false`
+
如果集群是双节点集群，则会显示一条消息，警告您禁用自动交还会阻止管理集群服务在发生交替故障时联机。输入 ... `y` 以继续。

. 验证配对节点是否已禁用自动交还： `storage failover show -node nodenameA -fields auto-giveback`
+
[listing]
----
...
1 entry was displayed.
----
. 运行以下命令两次、以确定要更新的节点当前是否正在为任何客户端提供服务： `system node run -node nodenameB -command uptime`
+
uptime 命令可显示节点自上次启动以来对 NFS ， SMB ， FC 和 iSCSI 客户端执行的操作总数。对于每个协议，您必须运行两次命令以确定操作计数是否在增加。如果它们不断增加，则表示节点当前正在为该协议的客户端提供服务。如果不增加，则节点当前不会为该协议的客户端提供服务。
...
cluster1::> system node run -node node1 -command uptime
  2:58pm up  7 days, 19:17 800001573 NFS ops, 1017333 CIFS ops, 0 HTTP ops, 40395 FCP ops, 32815 iSCSI ops
----
. 将所有数据生命周期迁移出节点： `network interface migrate-all -node nodenameB`
. 验证已迁移的任何生命周期的状态： `network interface show`
+
有关可用于验证 LIF 状态的参数的详细信息，请参见 network interface show 手册页。
...
+
.. 转到高级权限级别：``set -privilege advanced``
.. 验证节点的更新状态是否为complete： `system node upgrade-revert show -node nodenameB`
+
此状态应列为已完成。
...
4 entries were displayed.
----
. 如果您先前已确定此节点为客户端提供服务、请验证此节点是否正在为其先前提供的每个协议提供服务： `system node run -node nodenameB -command uptime`
+
更新期间，操作计数重置为零。
