...
---
= 在四节点或八节点 MetroCluster 配置上手动执行 ONTAP NDU
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
四节点或八节点 MetroCluster 配置的手动无中断升级 (NDU) 包括准备更新、同时更新一个或两个 DR 组中的每个 DR 对以及执行升级后任务。

.关于此任务
* 必须使用 ONTAP 命令行界面 (CLI) 来执行此任务。
* 此任务将对以下配置进行适用场景处理：
+
...
----
+
以下示例显示系统 image2（目标 ONTAP 映像）是 node_A_1 上的默认和当前版本：
...
----
+
以下示例显示系统 image2（目标 ONTAP 映像）是 node_B_1 上的默认和当前版本：
